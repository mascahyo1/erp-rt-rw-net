    ...
    <div class="space-y-6">
      ...
      <div v-if="!editMode" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden p-6 sm:p-8">
        <div class="flex items-center justify-between mb-6 pb-6 border-b border-gray-100 dark:border-gray-700">
          ...
          <button @click="startEdit" class="inline-flex items-center px-4 py-2.5 bg-sky-600 text-white text-sm font-medium rounded-lg hover:bg-sky-700 transition-colors shadow-sm"><i class="fas fa-edit mr-1.5"></i> Ubah Profil</button>
        </div>
      ...
      <form v-if="editMode" @submit.prevent="saveProfile" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-6 sm:p-8 space-y-4" data-testid="form-main">
        ...
        <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
          <h4 class="text-sm font-semibold text-gray-900 dark:text-white mb-3 flex items-center gap-2"><i class="fas fa-lock text-sky-500"></i> Ganti Password</h4>
          <button type="button" @click="showPassword = !showPassword" class="text-xs text-sky-600 dark:text-sky-400 hover:underline mb-3"><i :class="['fas mr-1', showPassword ? 'fa-chevron-up' : 'fa-chevron-down']"></i>{{ showPassword ? 'Sembunyikan' : 'Tampilkan' }} form ganti password</button>
          <div v-if="showPassword" class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Password Lama</label><div class="relative"><input v-model="passwordForm.current_password" :type="showPw ? 'text' : 'password'" class="w-full px-3 py-2.5 pr-10 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-sky-500 outline-none" /><button type="button" @click="showPw = !showPw" class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-sky-500 transition-colors" :title="showPw ? 'Sembunyikan' : 'Tampilkan'"><i :class="['fas text-sm', showPw ? 'fa-eye-slash' : 'fa-eye']"></i></button></div></div>
            <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Password Baru</label><div class="relative"><input v-model="passwordForm.password" :type="showPw ? 'text' : 'password'" class="w-full px-3 py-2.5 pr-10 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-sky-500 outline-none" /><button type="button" @click="showPw = !showPw" class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-sky-500 transition-colors" :title="showPw ? 'Sembunyikan' : 'Tampilkan'"><i :class="['fas text-sm', showPw ? 'fa-eye-slash' : 'fa-eye']"></i></button></div></div>
            <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Konfirmasi Password</label><div class="relative"><input v-model="passwordForm.password_confirmation" :type="showPw ? 'text' : 'password'" class="w-full px-3 py-2.5 pr-10 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-sky-500 outline-none" /><button type="button" @click="showPw = !showPw" class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-sky-500 transition-colors" :title="showPw ? 'Sembunyikan' : 'Tampilkan'"><i :class="['fas text-sm', showPw ? 'fa-eye-slash' : 'fa-eye']"></i></button></div></div>
            <div class="sm:col-span-3"><button type="button" @click="changePassword" class="px-4 py-2 bg-sky-600 text-white text-sm font-medium rounded-lg hover:bg-sky-700 shadow-sm"><i class="fas fa-key mr-1.5"></i> Ubah Password</button></div>
          </div>
        </div>

        <div class="flex items-center gap-2 pt-2">
          <button type="submit" class="px-6 py-2.5 bg-sky-600 text-white text-sm font-medium rounded-lg hover:bg-sky-700 transition-colors shadow-sm" data-testid="btn-simpan"><i class="fas fa-save mr-1.5"></i> Simpan</button>
