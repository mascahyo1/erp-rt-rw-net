...
function submitEdit() { editForm.transform(data => ({...data, _method: 'PUT'})).post('/operator-perusahaan/langganan-customer/' + selectedItem.value.id, { preserveState: true, preserveScroll: true, onSuccess: () => { showEditModal.value = false; fetchData(); toast.success('Langganan berhasil diperbarui.'); }, onError: () => toast.error('Validasi gagal: ' + errorSummary(editForm.errors), 6000) }); }
  ...
  <div>
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><h2 class="text-2xl font-bold text-gray-900 dark:text-white">Langganan Customer</h2><p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Kelola data pemasangan paket internet customer.</p></div><div class="flex flex-wrap items-center gap-2">
          ...
          <button v-if="can('langganan.export')" @click="exportAll" class="inline-flex items-center px-4 py-2.5 bg-violet-600 text-white text-sm font-medium rounded-lg hover:bg-violet-700 transition-colors shadow-sm" data-testid="btn-export"><i class="fas fa-file-export mr-1.5"></i> Export</button>
          ...
          <button v-if="can('langganan.create') && terhapusFilter !== 'ya'" @click="openCreate" class="inline-flex items-center px-4 py-2.5 bg-sky-600 text-white text-sm font-medium rounded-lg hover:bg-sky-700 transition-colors shadow-sm" data-testid="btn-tambah"><i class="fas fa-plus mr-1.5"></i> Tambah Langganan</button>
        </div></div>
      ...
      <div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showCreateModal = false">
        ...
        <form @submit.prevent="submitCreate" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
          ...
          <div class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll"><FormErrorSummary :errors="createForm.errors" />
            ...
            <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Status <span class="text-red-500">*</span></label><select v-model="createForm.internet_status" :class="['w-full px-3 py-2.5 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm outline-none', createForm.errors.internet_status ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500']"><option value="active">Aktif</option><option value="inactive">Nonaktif</option><option value="suspended">Suspend</option><option value="terminated">Terminasi</option></select><p v-if="createForm.errors.internet_status" class="text-red-500 text-xs mt-1">{{ createForm.errors.internet_status }}</p></div>
