...
function statusBadgeClass(s) {
    // payment_status_label: 'paid' | 'partial' | 'unpaid' (computed dari approved payments)
    if (s === 'paid' || s === 'lunas') return 'bg-emerald-100 text-emerald-700 dark:!bg-emerald-900/30 dark:!text-emerald-300';
    if (s === 'partial' || s === 'sebagian') return 'bg-amber-100 text-amber-700 dark:!bg-amber-900/30 dark:!text-amber-300';
    if (s === 'unpaid' || s === 'belum_bayar') return 'bg-red-100 text-red-700 dark:!bg-red-900/30 dark:!text-red-300';
...
async function openDetail(item) {
    ...
    try {
        const res = await fetch(`/api/tagihan/${item.id}/payments`, {
...
<template>
  <div>
    ...
    <!-- DETAIL MODAL -->
    <Teleport to="body"><Transition name="modal"><div v-if="showDetailModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showDetailModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg"><div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white">Detail Tagihan</h3><button @click="showDetailModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:text-white dark:hover:bg-gray-700"><i class="fas fa-times"></i></button></div><div class="px-6 py-5 space-y-3 dark:text-white" v-if="selectedItem"><div class="grid grid-cols-2 gap-3"><div><label class="text-xs text-gray-500 dark:text-gray-400">No. Invoice</label><p class="font-mono font-medium">{{ selectedItem.invoice_number }}</p></div><div><label class="text-xs text-gray-500 dark:text-gray-400">Pelanggan</label><p class="font-medium">{{ selectedItem.customer_name }}</p></div><div><label class="text-xs text-gray-500 dark:text-gray-400">No. Langganan</label><p class="font-mono">{{ selectedItem.account_number || '-' }}</p></div><div><label class="text-xs text-gray-500 dark:text-gray-400">Kode Pelanggan</label><p class="font-mono">{{ selectedItem.customer_code || '-' }}</p></div><div><label class="text-xs text-gray-500 dark:text-gray-400">Awal Usage</label><p>{{ selectedItem.usage_start_date || '-' }}</p></div><div><label class="text-xs text-gray-500 dark:text-gray-400">Akhir Usage</label><p>{{ selectedItem.usage_end_date || '-' }}</p></div><div><label class="text-xs text-gray-500 dark:text-gray-400">Total</label><p class="font-mono">Rp {{ Number(selectedItem.total_amount || 0).toLocaleString('id') }}</p></div><div><label class="text-xs text-gray-500 dark:text-gray-400">Diskon</label><p class="font-mono">Rp {{ Number(selectedItem.discount_amount || 0).toLocaleString('id') }}</p></div><div><label class="text-xs text-gray-500 dark:text-gray-400">Pajak</label><p class="font-mono">Rp {{ Number(selectedItem.tax_amount || 0).toLocaleString('id') }}</p></div><div><label class="text-xs text-gray-500 dark:text-gray-400">Grand Total</label><p class="font-mono font-semibold">Rp {{ Number(selectedItem.grand_total || 0).toLocaleString('id') }}</p></div><div><label class="text-xs text-gray-500 dark:text-gray-400">Jatuh Tempo</label><p>{{ selectedItem.due_date || '-' }}</p></div><div><label class="text-xs text-gray-500 dark:text-gray-400">Status</label><p><span :class="['inline-flex px-2.5 py-0.5 rounded text-xs font-medium', statusBadgeClass(selectedItem.payment_status)]">{{ statusLabel(selectedItem.payment_status) }}</span></p></div><div><label class="text-xs text-gray-500 dark:text-gray-400">Dibayar Pada</label><p>{{ selectedItem.paid_at || '-' }}</p></div><div><label class="text-xs text-gray-500 dark:text-gray-400">Deskripsi</label><p>{{ selectedItem.description || '-' }}</p></div></div><div class="pt-3 mt-1 border-t border-gray-200 dark:border-gray-700"><div class="flex items-center justify-between mb-2"><h4 class="text-sm font-semibold text-gray-900 dark:text-white">Riwayat Pembayaran</h4><span v-if="paymentHistory.payments.length > 0" :class="['inline-flex px-2 py-0.5 rounded-full text-xs font-medium', statusBadgeClass(paymentHistory.payment_status_label)]">{{ statusLabel(paymentHistory.payment_status_label) }}</span></div><div v-if="paymentHistoryLoading" class="text-xs text-gray-500 dark:text-gray-400 py-2">Memuat...</div><div v-else-if="paymentHistory.payments.length === 0" class="text-xs text-gray-500 dark:text-gray-400 italic py-2">Belum ada pembayaran.</div><div v-else class="overflow-x-auto"><table class="w-full text-xs"><thead><tr class="bg-gray-50 dark:bg-gray-900"><th class="px-2 py-1.5 text-left font-semibold text-gray-600 dark:text-gray-400">Kode Pembayaran</th><th class="px-2 py-1.5 text-center font-semibold text-gray-600 dark:text-gray-400">Tgl Bayar</th><th class="px-2 py-1.5 text-right font-semibold text-gray-600 dark:text-gray-400">Nominal</th></tr></thead><tbody><tr v-for="(p, idx) in paymentHistory.payments" :key="p.id" :class="['border-b border-gray-100 dark:border-gray-700', idx % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50/50 dark:bg-gray-900/30']"><td class="px-2 py-1.5 font-mono">{{ p.code }}</td><td class="px-2 py-1.5 text-center">{{ p.payment_date }}</td><td class="px-2 py-1.5 text-right font-mono">Rp {{ Number(p.amount_paid || 0).toLocaleString('id') }}</td></tr></tbody><tfoot><tr class="border-t-2 border-gray-300 dark:border-gray-600"><td colspan="2" class="px-2 py-1.5 text-right text-xs font-semibold">Total Pembayaran:</td><td class="px-2 py-1.5 text-right font-mono font-semibold text-emerald-700 dark:text-emerald-400">Rp {{ Number(paymentHistory.total_paid || 0).toLocaleString('id') }}</td></tr><tr><td colspan="2" class="px-2 py-1.5 text-right text-xs font-semibold">Sisa yang belum dibayar:</td><td class="px-2 py-1.5 text-right font-mono font-semibold" :class="paymentHistory.remaining > 0 ? 'text-red-700 dark:text-red-400' : 'text-emerald-700 dark:text-emerald-400'">Rp {{ Number(paymentHistory.remaining || 0).toLocaleString('id') }}</td></tr></tfoot></table></div></div><div v-if="can('tagihan.export')" class="flex items-center justify-end gap-2 pt-4 border-t border-gray-200 dark:border-gray-700 mt-4"><a :href="`/operator-perusahaan/tagihan/${selectedItem.id}/export-pdf`" target="_blank" class="inline-flex items-center px-3 py-1.5 bg-red-600 text-white text-xs font-medium rounded-lg hover:bg-red-700 transition-colors"><i class="fas fa-file-pdf mr-1.5"></i>Export PDF</a><a :href="`/operator-perusahaan/tagihan/${selectedItem.id}/export-word`" target="_blank" class="inline-flex items-center px-3 py-1.5 bg-blue-600 text-white text-xs font-medium rounded-lg hover:bg-blue-700 transition-colors"><i class="fas fa-file-word mr-1.5"></i>Export Word</a></div></div></div></div></Transition></Teleport>
