...
function statusBadgeClass(s) { if (s === 'paid' || s === 'lunas') return 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400'; if (s === 'unpaid' || s === 'belum_bayar') return 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'; return 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'; }
function statusLabel(s) { if (s === 'paid') return 'Lunas'; if (s === 'unpaid') return 'Belum Bayar'; if (s === 'overdue') return 'Kadaluarsa'; return s; }

const createForm = useForm({ cust_internet_id: '', total_amount: '', discount_amount: '', tax_amount: '', due_date: '', description: '' });
const editForm = useForm({ cust_internet_id: '', total_amount: '', discount_amount: '', tax_amount: '', due_date: '', description: '' });
...
function openEdit(item) { editForm.defaults({ cust_internet_id: item.cust_internet_id, total_amount: item.total_amount, discount_amount: item.discount_amount, tax_amount: item.tax_amount, due_date: item.due_date, description: item.description }); editForm.reset(); selectedItem.value = item; showEditModal.value = true; }
...
<template>
  <div>
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><h2 class="text-2xl font-bold text-gray-900 dark:text-white">Tagihan</h2><p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Kelola tagihan bulanan pelanggan.</p></div><button v-if="terhapusFilter !== 'ya'" @click="openCreate" class="inline-flex items-center px-4 py-2.5 bg-sky-600 text-white text-sm font-medium rounded-lg hover:bg-sky-700 transition-colors shadow-sm"><i class="fas fa-plus mr-1.5"></i> Tambah Tagihan</button></div>
      ...
      <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
        ...
        <tbody>
          ...
          <tr v-for="item in items" :key="item.id" class="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors" :class="{'opacity-60': item.dihapus}">
            ...
            <td class="px-4 py-3 text-center"><span :class="['inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium', statusBadgeClass(item.payment_status)]">{{ statusLabel(item.payment_status) }}</span></td>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showDetailModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showDetailModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg"><div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold">Detail Tagihan</h3><button @click="showDetailModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700"><i class="fas fa-times"></i></button></div><div class="px-6 py-5 space-y-3" v-if="selectedItem"><div class="grid grid-cols-2 gap-3"><div><label class="text-xs text-gray-500">No. Invoice</label><p class="font-mono font-medium">{{ selectedItem.invoice_number }}</p></div><div><label class="text-xs text-gray-500">Pelanggan</label><p class="font-medium">{{ selectedItem.customer_name }}</p></div><div><label class="text-xs text-gray-500">Total</label><p class="font-mono">Rp {{ Number(selectedItem.total_amount || 0).toLocaleString('id') }}</p></div><div><label class="text-xs text-gray-500">Diskon</label><p class="font-mono">Rp {{ Number(selectedItem.discount_amount || 0).toLocaleString('id') }}</p></div><div><label class="text-xs text-gray-500">Pajak</label><p class="font-mono">Rp {{ Number(selectedItem.tax_amount || 0).toLocaleString('id') }}</p></div><div><label class="text-xs text-gray-500">Grand Total</label><p class="font-mono font-semibold">Rp {{ Number(selectedItem.grand_total || 0).toLocaleString('id') }}</p></div><div><label class="text-xs text-gray-500">Jatuh Tempo</label><p>{{ selectedItem.due_date }}</p></div><div><label class="text-xs text-gray-500">Status</label><p><span :class="['px-2 py-0.5 rounded text-xs font-medium', statusBadgeClass(selectedItem.payment_status)]">{{ statusLabel(selectedItem.payment_status) }}</span></p></div><div><label class="text-xs text-gray-500">Dibayar Pada</label><p>{{ selectedItem.paid_at || '-' }}</p></div><div><label class="text-xs text-gray-500">Deskripsi</label><p>{{ selectedItem.description || '-' }}</p></div></div></div></div></div></Transition></Teleport>
    <Teleport to="body"><Transition name="modal"><div v-if="showCreateModal || showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showCreateModal = showEditModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto"><div class="sticky top-0 z-10 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-t-2xl"><h3 class="text-lg font-semibold"><i :class="['fas mr-2', showCreateModal ? 'fa-plus text-emerald-500' : 'fa-edit text-amber-500']"></i>{{ showCreateModal ? 'Tambah Tagihan' : 'Edit Tagihan' }}</h3><button @click="showCreateModal = showEditModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700"><i class="fas fa-times"></i></button></div><form class="px-6 py-5 space-y-4" @submit.prevent="showCreateModal ? submitCreate() : submitEdit()">
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Customer Internet ID <span class="text-red-500">*</span></label><input v-model="(showCreateModal ? createForm : editForm).cust_internet_id" type="text" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-sky-500 outline-none" /></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Total Tagihan (Rp) <span class="text-red-500">*</span></label><input v-model="(showCreateModal ? createForm : editForm).total_amount" type="number" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-sky-500 outline-none" /></div>
        <div class="grid grid-cols-2 gap-4"><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Diskon</label><input v-model="(showCreateModal ? createForm : editForm).discount_amount" type="number" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-sky-500 outline-none" /></div><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Pajak</label><input v-model="(showCreateModal ? createForm : editForm).tax_amount" type="number" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-sky-500 outline-none" /></div></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Jatuh Tempo <span class="text-red-500">*</span></label><input v-model="(showCreateModal ? createForm : editForm).due_date" type="date" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-sky-500 outline-none" /></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Deskripsi</label><textarea v-model="(showCreateModal ? createForm : editForm).description" rows="2" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-sky-500 outline-none"></textarea></div>
