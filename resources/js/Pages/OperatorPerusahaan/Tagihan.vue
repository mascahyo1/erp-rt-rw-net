...
function submitCreate() { createForm.post('/operator-perusahaan/tagihan', { onSuccess: () => { showCreateModal.value = false; fetchData(); toast.success('Tagihan berhasil ditambahkan.'); }, onError: () => toast.error('Validasi gagal: ' + errorSummary(createForm.errors), 6000) }); }
...
function submitEdit() { editForm.transform(data => ({...data, _method: 'PUT'})).post('/operator-perusahaan/tagihan/' + selectedItem.value.id, { onSuccess: () => { showEditModal.value = false; fetchData(); toast.success('Tagihan berhasil diperbarui.'); }, onError: () => toast.error('Validasi gagal: ' + errorSummary(editForm.errors), 6000) }); }
...
function submitGenerate() { generateForm.post('/operator-perusahaan/tagihan/generate', { onSuccess: () => { showGenerateModal.value = false; fetchData(); toast.success('Tagihan berhasil digenerate.'); }, onError: () => toast.error('Validasi gagal: ' + errorSummary(generateForm.errors), 6000) }); }

function openImport() { importForm.reset(); showImportModal.value = true; }
function submitImport() { importing.value = true; importForm.post('/operator-perusahaan/tagihan/import', { onSuccess: () => { showImportModal.value = false; importing.value = false; fetchData(); toast.success('Import berhasil.'); }, onError: () => { importing.value = false; toast.error('Validasi gagal: ' + errorSummary(importForm.errors), 6000); } }); }
...
<template>
  <div>
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          <button v-if="can('tagihan.import')" @click="openImport" class="inline-flex items-center px-3 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 transition-colors shadow-sm" data-testid="btn-import"><i class="fas fa-file-import mr-1.5"></i> Import</button>
          ...
          <button v-if="can('tagihan.create')" @click="openCreate" class="inline-flex items-center px-4 py-2.5 bg-sky-600 text-white text-sm font-medium rounded-lg hover:bg-sky-700 transition-colors shadow-sm" data-testid="btn-tambah"><i class="fas fa-plus mr-1.5"></i> Tambah</button>
        </div>
      </div>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showCreateModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><form @submit.prevent="submitCreate" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col"><div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-plus text-emerald-500 mr-2"></i>Tambah Tagihan</h3><button type="button" @click="showCreateModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><div class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll">
        ...
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Langganan <span class="text-red-500">*</span></label><SearchableSelectAjax v-model="createForm.cust_internet_id" url="/operator-perusahaan/api/search/langganans" placeholder="— Pilih Langganan —" :error="!!createForm.errors.cust_internet_id" /><p v-if="createForm.errors.cust_internet_id" class="text-red-500 text-xs mt-1">{{ createForm.errors.cust_internet_id }}</p></div>
        <div class="grid grid-cols-2 gap-3"><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Awal Usage</label><input v-model="createForm.usage_start_date" type="date" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white dark:[color-scheme:dark]', createForm.errors.usage_start_date ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="createForm.errors.usage_start_date" class="text-red-500 text-xs mt-1">{{ createForm.errors.usage_start_date }}</p></div><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Akhir Usage</label><input v-model="createForm.usage_end_date" type="date" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white dark:[color-scheme:dark]', createForm.errors.usage_end_date ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="createForm.errors.usage_end_date" class="text-red-500 text-xs mt-1">{{ createForm.errors.usage_end_date }}</p></div></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Total Tagihan <span class="text-red-500">*</span></label><input v-model="createForm.total_amount" type="number" placeholder="0" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', createForm.errors.total_amount ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="createForm.errors.total_amount" class="text-red-500 text-xs mt-1">{{ createForm.errors.total_amount }}</p></div>
        <div class="grid grid-cols-2 gap-3"><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Diskon</label><input v-model="createForm.discount_amount" type="number" placeholder="0" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', createForm.errors.discount_amount ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="createForm.errors.discount_amount" class="text-red-500 text-xs mt-1">{{ createForm.errors.discount_amount }}</p></div><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Pajak</label><input v-model="createForm.tax_amount" type="number" placeholder="0" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', createForm.errors.tax_amount ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="createForm.errors.tax_amount" class="text-red-500 text-xs mt-1">{{ createForm.errors.tax_amount }}</p></div></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Jatuh Tempo</label><input v-model="createForm.due_date" type="date" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white dark:[color-scheme:dark]', createForm.errors.due_date ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="createForm.errors.due_date" class="text-red-500 text-xs mt-1">{{ createForm.errors.due_date }}</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Deskripsi</label><textarea v-model="createForm.description" rows="2" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors resize-none bg-white dark:bg-gray-900 text-gray-900 dark:text-white', createForm.errors.description ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']"></textarea><p v-if="createForm.errors.description" class="text-red-500 text-xs mt-1">{{ createForm.errors.description }}</p></div>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showEditModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><form @submit.prevent="submitEdit" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col"><div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-edit text-sky-500 mr-2"></i>Edit Tagihan</h3><button type="button" @click="showEditModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><div class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll">
        ...
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Langganan <span class="text-red-500">*</span></label><SearchableSelectAjax v-model="editForm.cust_internet_id" url="/operator-perusahaan/api/search/langganans" placeholder="— Pilih Langganan —" :error="!!editForm.errors.cust_internet_id" :selected-label="selectedItem?.customer_name" /><p v-if="editForm.errors.cust_internet_id" class="text-red-500 text-xs mt-1">{{ editForm.errors.cust_internet_id }}</p></div>
        <div class="grid grid-cols-2 gap-3"><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Awal Usage</label><input v-model="editForm.usage_start_date" type="date" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white dark:[color-scheme:dark]', editForm.errors.usage_start_date ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="editForm.errors.usage_start_date" class="text-red-500 text-xs mt-1">{{ editForm.errors.usage_start_date }}</p></div><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Akhir Usage</label><input v-model="editForm.usage_end_date" type="date" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white dark:[color-scheme:dark]', editForm.errors.usage_end_date ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="editForm.errors.usage_end_date" class="text-red-500 text-xs mt-1">{{ editForm.errors.usage_end_date }}</p></div></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Total Tagihan <span class="text-red-500">*</span></label><input v-model="editForm.total_amount" type="number" placeholder="0" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', editForm.errors.total_amount ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="editForm.errors.total_amount" class="text-red-500 text-xs mt-1">{{ editForm.errors.total_amount }}</p></div>
        <div class="grid grid-cols-2 gap-3"><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Diskon</label><input v-model="editForm.discount_amount" type="number" placeholder="0" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', editForm.errors.discount_amount ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="editForm.errors.discount_amount" class="text-red-500 text-xs mt-1">{{ editForm.errors.discount_amount }}</p></div><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Pajak</label><input v-model="editForm.tax_amount" type="number" placeholder="0" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', editForm.errors.tax_amount ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="editForm.errors.tax_amount" class="text-red-500 text-xs mt-1">{{ editForm.errors.tax_amount }}</p></div></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Jatuh Tempo</label><input v-model="editForm.due_date" type="date" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white dark:[color-scheme:dark]', editForm.errors.due_date ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="editForm.errors.due_date" class="text-red-500 text-xs mt-1">{{ editForm.errors.due_date }}</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Deskripsi</label><textarea v-model="editForm.description" rows="2" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors resize-none bg-white dark:bg-gray-900 text-gray-900 dark:text-white', editForm.errors.description ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']"></textarea><p v-if="editForm.errors.description" class="text-red-500 text-xs mt-1">{{ editForm.errors.description }}</p></div>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showGenerateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showGenerateModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><form @submit.prevent="submitGenerate" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md"><div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-magic text-purple-500 mr-2"></i>Generate Tagihan Massal</h3><button type="button" @click="showGenerateModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><div class="px-6 py-5 space-y-4">
        <FormErrorSummary :errors="generateForm.errors" />
        ...
        <div>
          ...
          <div :class="['grid grid-cols-4 gap-2', generateForm.errors.cycle ? 'rounded-lg ring-2 ring-red-400 p-2' : '']">
        ...
        <div v-if="generateForm.cycle === 'daily' || generateForm.cycle === 'weekly'">
          ...
          <input v-model="generateForm.usage_date" type="date" :min="generateForm.cycle === 'weekly' ? null : new Date().toISOString().slice(0, 10)" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white dark:[color-scheme:dark]', generateForm.errors.usage_date ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="generateForm.errors.usage_date" class="text-red-500 text-xs mt-1">{{ generateForm.errors.usage_date }}</p>
        ...
        <div v-if="generateForm.cycle === 'monthly'" class="grid grid-cols-2 gap-3">
          <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Tahun <span class="text-red-500">*</span></label><input v-model="generateForm.period_year" type="number" min="2020" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', generateForm.errors.period_year ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="generateForm.errors.period_year" class="text-red-500 text-xs mt-1">{{ generateForm.errors.period_year }}</p></div>
          <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Bulan <span class="text-red-500">*</span></label><select v-model="generateForm.period_month" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', generateForm.errors.period_month ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']"><option v-for="m in 12" :key="m" :value="m">{{ new Date(2000, m-1, 1).toLocaleString('id', { month: 'long' }) }}</option></select><p v-if="generateForm.errors.period_month" class="text-red-500 text-xs mt-1">{{ generateForm.errors.period_month }}</p></div>
        </div>
        <div v-if="generateForm.cycle === 'yearly'">
          ...
          <input v-model="generateForm.period_year" type="number" min="2020" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', generateForm.errors.period_year ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="generateForm.errors.period_year" class="text-red-500 text-xs mt-1">{{ generateForm.errors.period_year }}</p>
        </div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Jatuh Tempo (opsional)</label><input v-model="generateForm.due_date" type="date" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white dark:[color-scheme:dark]', generateForm.errors.due_date ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="generateForm.errors.due_date" class="text-red-500 text-xs mt-1">{{ generateForm.errors.due_date }}</p><p class="text-xs text-gray-500 mt-1">Kosongkan untuk menggunakan default 30 hari setelah akhir periode</p></div>
