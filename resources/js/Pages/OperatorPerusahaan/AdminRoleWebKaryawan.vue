...
function submitImport() { importForm.post('/operator-perusahaan/admin-role-web-karyawan/import', { preserveState: true, preserveScroll: true, onSuccess: () => { showImportModal.value = false; fetchData(); toast.success('Import berhasil.') }, onError: () => toast.error('Validasi gagal: ' + errorSummary(importForm.errors), 6000) }); }
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md sm:max-w-lg max-h-[90vh] flex flex-col">
          ...
          <form class="flex-1 overflow-y-auto px-6 py-5 space-y-4 modal-scroll" @submit.prevent="showCreateModal ? submitCreate() : submitEdit()">
            ...
            <div v-if="showCreateModal">
              ...
              <SearchableSelectAjax v-model="createForm.karyawan_id" :url="KARYAWAN_URL" placeholder="— Pilih Karyawan —" test-id="select-create-karyawan" :class="createForm.errors.karyawan_id ? 'ring-2 ring-red-400 rounded-lg' : ''" />
        ...
        <form @submit.prevent="submitBulkAssign" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col" data-testid="modal-bulk-assign">
          ...
          <div class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll">
            ...
            <div>
              ...
              <MultiSelectAjax v-model="bulkAssignForm.karyawan_ids" :url="KARYAWAN_URL" placeholder="— Pilih Karyawan —" test-id="multiselect-bulk-karyawan" :class="bulkAssignForm.errors.karyawan_ids ? 'ring-2 ring-red-400 rounded-lg' : ''" />
