...
const roleList = ['Supervisor', 'Kolektor', 'Staff'];
const adminOptions = computed(() => adminList.map(a => ({ value: a, label: a })));
const roleOptions = computed(() => roleList.map(r => ({ value: r, label: r })));

const mappings = ref([
  { id: 1, admin: 'Ahmad Fauzi', role: 'Supervisor', status: 'Aktif', created_at: '2025-01-15' },
  { id: 2, admin: 'Siti Nuraini', role: 'Kolektor', status: 'Aktif', created_at: '2025-02-20' },
  { id: 3, admin: 'Budi Santoso', role: 'Kolektor', status: 'Aktif', created_at: '2025-03-10' },
  { id: 4, admin: 'Dewi Lestari', role: 'Staff', status: 'Nonaktif', created_at: '2025-04-05' },
  { id: 5, admin: 'Rudi Hermawan', role: 'Staff', status: 'Aktif', created_at: '2025-05-01' },
]);

const searchInput = ref(''); const search = ref(''); const statusFilter = ref(''); const roleFilter = ref(''); const currentPage = ref(1); const perPage = ref(5); const perPageOptions = [5, 10, 25, 50, 100];
...
const form = ref({ id: null, admin: '', role: '', status: 'Aktif' }); const formErrors = ref({}); const nextId = ref(6);

function applySearch() { search.value = searchInput.value; currentPage.value = 1; } function clearSearch() { searchInput.value = ''; search.value = ''; currentPage.value = 1; }
function applyStatusFilter(s) { statusFilter.value = s; currentPage.value = 1; } function applyRoleFilter(r) { roleFilter.value = r; currentPage.value = 1; } function changePerPage(n) { perPage.value = n; currentPage.value = 1; }
...
function bulkDelete() { mappings.value = mappings.value.filter(m => !selectedIds.value.includes(m.id)); selectedIds.value = []; selectAll.value = false; } function bulkSetStatus(s) { mappings.value.forEach(m => { if (selectedIds.value.includes(m.id)) m.status = s; }); selectedIds.value = []; selectAll.value = false; }
...
const filteredMappings = computed(() => {
  let r = [...mappings.value]; if (search.value) { const q = search.value.toLowerCase(); r = r.filter(x => x.admin.toLowerCase().includes(q) || x.role.toLowerCase().includes(q)); }
  if (statusFilter.value) r = r.filter(x => x.status === statusFilter.value); if (roleFilter.value) r = r.filter(x => x.role === roleFilter.value);
...
const totalPages = computed(() => Math.ceil(filteredMappings.value.length / perPage.value) || 1); const paginatedMappings = computed(() => { const s = (currentPage.value - 1) * perPage.value; return filteredMappings.value.slice(s, s + perPage.value); });
...
function openCreate() { form.value = { id: null, admin: '', role: '', status: 'Aktif' }; formErrors.value = {}; showCreateModal.value = true; }
function openDetail(m) { selectedItem.value = m; showDetailModal.value = true; } function openEdit(m) { form.value = { ...m }; formErrors.value = {}; showEditModal.value = true; } function openDelete(m) { selectedItem.value = m; showDeleteModal.value = true; }
function validateForm() { const e = {}; if (!form.value.admin) e.admin = 'Admin wajib dipilih'; if (!form.value.role) e.role = 'Role wajib dipilih'; formErrors.value = e; return Object.keys(e).length === 0; }
...
function confirmDelete() { mappings.value = mappings.value.filter(m => m.id !== selectedItem.value.id); showDeleteModal.value = false; if (paginatedMappings.value.length === 0 && currentPage.value > 1) currentPage.value--; } function goToPage(p) { currentPage.value = Math.max(1, Math.min(p, totalPages.value)); }
</script>
  ...
  <div>
    ...
    <div class="space-y-6">
      <nav class="flex items-center gap-1.5 text-sm">
        <Link href="/operator-perusahaan/dashboard" class="text-gray-500 dark:text-gray-400 hover:text-sky-600 dark:hover:text-sky-400 transition-colors">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Pemetaan role web karyawan untuk admin perusahaan.</p>
        </div>
      ...
      <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          <div class="relative w-full sm:w-72">
            ...
            <input v-model="searchInput" type="text" placeholder="Cari admin atau role..." class="w-full pl-10 pr-16 py-2.5 rounded-lg border" @keydown.enter="applySearch" />
            <div class="absolute inset-y-0 right-0 flex items-center gap-1 pr-1.5">
              <button v-if="searchInput" @click="clearSearch" class="p-1 rounded text-gray-400 hover:text-gray-600 dark:hover:text-white transition-colors" title="Clear">
                <i class="fas fa-times text-xs"></i>
              </button>
              <button @click="applySearch" class="px-2 py-1 rounded bg-sky-600 text-white hover:bg-sky-700 transition-colors" title="Cari">
                <i class="fas fa-search text-xs"></i>
              </button>
            </div>
          </div>
          <SearchableSelect :model-value="roleFilter" :options="roleOptions" placeholder="Semua Role" @update:model-value="applyRoleFilter" />
          <div class="flex gap-1 flex-wrap">
            ...
            >
              ...
              <i v-else-if="s.v === 'Nonaktif'" class="fas fa-ban mr-1"></i>
              {{ s.l }}
            </button>
          </div>
        </div>
      ...
      <div v-if="selectedIds.length > 0" class="flex items-center justify-between px-4 py-3 bg-sky-50 dark:bg-sky-900/20 border border-sky-200 dark:border-sky-800 rounded-xl shadow-sm">
        ...
        <div class="flex items-center gap-2">
          <button @click="bulkSetStatus('Aktif')" class="px-3 py-1.5 text-xs font-medium rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 transition-colors">
            <i class="fas fa-check mr-1"></i> Aktifkan
          </button>
          <button @click="bulkSetStatus('Nonaktif')" class="px-3 py-1.5 text-xs font-medium rounded-lg bg-amber-600 text-white hover:bg-amber-700 transition-colors">
            <i class="fas fa-ban mr-1"></i> Nonaktifkan
          </button>
          <button @click="bulkDelete()" class="px-3 py-1.5 text-xs font-medium rounded-lg bg-red-600 text-white hover:bg-red-700 transition-colors">
      ...
      <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
          ...
          <table class="w-full text-sm min-w-[600px]">
            <thead class="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
              <tr>
                ...
                <th @click="sort('admin')" class="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-400 cursor-pointer select-none hover:text-gray-900 dark:hover:text-white transition-colors">
                  <span class="inline-flex items-center gap-1">
                    Admin
                    <span v-if="sortOrder('admin')" class="inline-flex items-center gap-0.5 text-sky-500 dark:text-sky-400">
                      <i :class="['fas', sortIcon('admin'), 'text-[10px]']"></i>
                      <span class="text-[10px] font-bold leading-none">{{ sortOrder('admin') }}</span>
                    </span>
                    <i v-else class="fas fa-sort text-[10px] text-gray-400 dark:text-gray-500"></i>
                  </span>
                </th>
                <th @click="sort('role')" class="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-400 cursor-pointer select-none hover:text-gray-900 dark:hover:text-white transition-colors">
                  <span class="inline-flex items-center gap-1">
                    Role
                    <span v-if="sortOrder('role')" class="inline-flex items-center gap-0.5 text-sky-500 dark:text-sky-400">
                      <i :class="['fas', sortIcon('role'), 'text-[10px]']"></i>
                      <span class="text-[10px] font-bold leading-none">{{ sortOrder('role') }}</span>
                    </span>
                    <i v-else class="fas fa-sort text-[10px] text-gray-400 dark:text-gray-500"></i>
                  </span>
                </th>
                <th @click="sort('status')" class="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-400 cursor-pointer select-none hover:text-gray-900 dark:hover:text-white transition-colors">
                  <span class="inline-flex items-center gap-1">
                    Status
                    <span v-if="sortOrder('status')" class="inline-flex items-center gap-0.5 text-sky-500 dark:text-sky-400">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-if="paginatedMappings.length === 0">
                <td colspan="5" class="px-4 py-16 text-center text-gray-500 dark:text-gray-400">
                  <i class="fas fa-user-gear text-3xl mb-2 block opacity-40"></i>
                  Tidak ada mapping.
                </td>
              </tr>
              <tr v-for="m in paginatedMappings" :key="m.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                <td class="px-4 py-3">
                  <input :checked="selectedIds.includes(m.id)" type="checkbox" @change="toggleSelect(m.id)" class="rounded border-gray-300 dark:border-gray-600 text-sky-600 focus:ring-sky-500" />
                </td>
                <td class="px-4 py-3">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="font-medium text-gray-900 dark:text-white">{{ m.admin }}</span>
                  </div>
                </td>
                <td class="px-4 py-3">
                  <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">
                    <i class="fas fa-tag text-[10px]"></i> {{ m.role }}
                  </span>
                </td>
                <td class="px-4 py-3 whitespace-nowrap">
                  <span :class="['inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium', statusBadge(m.status)]">{{ m.status }}</span>
                </td>
                <td class="px-4 py-3 whitespace-nowrap">
                  <div class="flex items-center justify-end gap-1">
                    <button @click="openDetail(m)" class="p-2 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-sky-600 dark:hover:text-sky-400 transition-colors" title="Detail">
                      <i class="fas fa-eye text-sm"></i>
                    </button>
                    <button @click="openEdit(m)" class="p-2 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-sky-600 dark:hover:text-sky-400 transition-colors" title="Edit">
                      <i class="fas fa-edit text-sm"></i>
                    </button>
                    <button @click="openDelete(m)" class="p-2 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-red-600 dark:hover:text-red-400 transition-colors" title="Hapus">
        ...
        <div class="px-4 py-3 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
            <span>Tampilkan</span>
            <select :value="perPage" @change="changePerPage(Number($event.target.value))" class="min-w-[65px] px-2 py-1 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm">
              <option v-for="n in perPageOptions" :key="n" :value="n">{{ n }}</option>
            </select>
            <span>dari {{ filteredMappings.length }} data</span>
          </div>
          ...
          <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
              <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                <i class="fas fa-eye mr-2 text-sky-500"></i>Detail Mapping
              </h3>
            ...
            <div class="px-6 py-5 space-y-4">
              <div class="flex items-center gap-4 pb-4 border-b border-gray-100 dark:border-gray-700">
                <div class="w-16 h-16 rounded-full bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center text-white text-2xl font-bold shrink-0">
                  {{ selectedItem?.admin?.charAt(0) }}
                </div>
                <div>
                  <h4 class="text-xl font-bold text-gray-900 dark:text-white">{{ selectedItem?.admin }}</h4>
                  <span :class="['inline-flex mt-1 px-2.5 py-0.5 rounded-full text-xs font-medium', statusBadge(selectedItem?.status)]">{{ selectedItem?.status }}</span>
                </div>
              </div>
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Role Web Karyawan</label>
                  <p class="text-sm text-gray-900 dark:text-white mt-1">
                    <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">
                      <i class="fas fa-tag text-[10px]"></i> {{ selectedItem?.role }}
                    </span>
                  </p>
                </div>
                <div>
                  <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Tanggal Dibuat</label>
                  <p class="text-sm text-gray-900 dark:text-white mt-1">{{ selectedItem?.created_at }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div class="sticky top-0 z-10 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-t-2xl">
              <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                ...
                {{ showCreateModal ? 'Tambah Mapping' : 'Edit Mapping' }}
              </h3>
            ...
            <form class="px-6 py-5 space-y-4" @submit.prevent="showCreateModal ? saveCreate() : saveEdit()">
              <div>
                ...
                <SearchableSelect v-model="form.admin" :options="adminOptions" placeholder="— Pilih Admin —" />
                <p v-if="formErrors.admin" class="text-red-500 text-xs mt-1">{{ formErrors.admin }}</p>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Role Web Karyawan <span class="text-red-500">*</span></label>
              ...
              <div>
                ...
                <select v-model="form.status" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm">
          ...
          <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div class="px-6 py-5 text-center">
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400">
                Anda akan menghapus <strong class="text-gray-700 dark:text-gray-300">{{ selectedItem?.admin }} — {{ selectedItem?.role }}</strong>.
              </p>
            </div>
