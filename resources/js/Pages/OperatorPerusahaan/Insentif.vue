...
function bulkSetStatus(s) { router.post('/operator-perusahaan/insentif/bulk-status', { ids: selectedIds.value, status: s }, { onSuccess: () => { selectedIds.value = []; selectAll.value = false; fetchData(); toast.success('Insentif berhasil ' + (s === 'Aktif' ? 'diaktifkan' : 'dinonaktifkan') + '.'); } }); }
function exportAll() { window.location.href = '/operator-perusahaan/insentif/export'; }
function openImport() { showImportModal.value = true; }
function submitImport() { router.post('/operator-perusahaan/insentif/import', { file: importFile.value }, { onSuccess: () => { showImportModal.value = false; importFile.value = null; fetchData(); toast.success('Import berhasil.'); }, onError: (e) => toast.error('Import gagal.') }); }
...
<template>
  <div>
      ...
      <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
        ...
        <tbody>
          ...
          <tr v-for="item in items" :key="item.id" class="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors" :class="{'opacity-60': item.dihapus}">
            <td class="px-4 py-3"><input :value="item.id" type="checkbox" @change="toggleSelect(item.id)" :checked="selectedIds.includes(item.id)" class="rounded border-gray-300 text-sky-600" /></td>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showImportModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showImportModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md"><div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold"><i class="fas fa-file-import mr-2 text-emerald-500"></i>Import Insentif</h3><button @click="showImportModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700"><i class="fas fa-times"></i></button></div><div class="px-6 py-5"><div class="mb-4"><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">File Excel</label><input type="file" ref="importFile" accept=".xlsx,.xls" class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-sky-50 file:text-sky-700 hover:file:bg-sky-100" /><p class="text-xs text-gray-400 mt-1">Format: .xlsx atau .xls</p></div><div class="flex justify-end gap-2"><button @click="showImportModal = false" class="px-4 py-2.5 bg-gray-200 dark:bg-gray-700 text-sm font-medium rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600">Batal</button><button @click="submitImport" class="px-6 py-2.5 bg-sky-600 text-white text-sm font-medium rounded-lg hover:bg-sky-700 shadow-sm"><i class="fas fa-upload mr-1.5"></i>Import</button></div></div></div></div></Transition></Teleport>
  </div>
</template>
