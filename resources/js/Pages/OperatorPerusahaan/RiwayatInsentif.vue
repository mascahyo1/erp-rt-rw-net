...
function submitEdit() {
  editForm.transform(data => ({...data, _method: 'PUT'})).post('/operator-perusahaan/riwayat-insentif/' + selectedItem.value.id, {
    preserveState: true, preserveScroll: true,
...
function submitImport() {
  ...
  importForm.post('/operator-perusahaan/riwayat-insentif/import', {
    ...
    onError: () => { importing.value = false; toast.error('Validasi gagal: ' + errorSummary(importForm.errors), 6000); }
  });
}
...
<template>
  <div>
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
          <button v-if="can('riwayat-insentif.import') && terhapusFilter !== 'ya'" @click="openImport" class="inline-flex items-center px-3 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 transition-colors shadow-sm" data-testid="btn-import"><i class="fas fa-file-import mr-1.5"></i> Import</button>
          ...
          <button v-if="can('riwayat-insentif.create') && terhapusFilter !== 'ya'" @click="openCreate" class="inline-flex items-center px-4 py-2.5 bg-sky-600 text-white text-sm font-medium rounded-lg hover:bg-sky-700 transition-colors shadow-sm" data-testid="btn-tambah"><i class="fas fa-plus mr-1.5"></i> Tambah Riwayat</button>
        </div>
      </div>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showCreateModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><form @submit.prevent="submitCreate" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col"><div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-plus text-emerald-500 mr-2"></i>Tambah Riwayat Insentif</h3><button type="button" @click="showCreateModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><div class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll">
        ...
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Kode Invoice <span class="text-red-500">*</span></label><input v-model="createForm.invoice_number" type="text" placeholder="Contoh: INV/2025/001" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', createForm.errors.invoice_number ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="createForm.errors.invoice_number" class="text-red-500 text-xs mt-1">{{ createForm.errors.invoice_number }}</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Insentif <span class="text-red-500">*</span></label><SearchableSelectAjax v-model="createForm.emp_incentive_id" url="/operator-perusahaan/api/search/incentives" placeholder="— Pilih Insentif —" :error="!!createForm.errors.emp_incentive_id" /><p v-if="createForm.errors.emp_incentive_id" class="text-red-500 text-xs mt-1">{{ createForm.errors.emp_incentive_id }}</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Invoice <span class="text-red-500">*</span></label><SearchableSelectAjax v-model="createForm.cust_internet_invcs_id" url="/operator-perusahaan/api/search/invoices" placeholder="— Pilih Invoice —" :error="!!createForm.errors.cust_internet_invcs_id" /><p v-if="createForm.errors.cust_internet_invcs_id" class="text-red-500 text-xs mt-1">{{ createForm.errors.cust_internet_invcs_id }}</p></div>
        <div class="grid grid-cols-2 gap-3"><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Jumlah <span class="text-red-500">*</span></label><input v-model="createForm.amount" type="number" placeholder="0" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', createForm.errors.amount ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="createForm.errors.amount" class="text-red-500 text-xs mt-1">{{ createForm.errors.amount }}</p></div><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Tanggal <span class="text-red-500">*</span></label><input v-model="createForm.date" type="date" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', createForm.errors.date ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="createForm.errors.date" class="text-red-500 text-xs mt-1">{{ createForm.errors.date }}</p></div></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Diajukan Untuk</label><SearchableSelectAjax v-model="createForm.submitted_by_id" url="/operator-perusahaan/api/search/employees" placeholder="— Pilih Karyawan —" :error="!!createForm.errors.submitted_by_id" :on-select="(opt) => { createForm.submitted_by_name = opt ? opt.label.split(' (')[0] : ''; }" /><p v-if="createForm.errors.submitted_by_id" class="text-red-500 text-xs mt-1">{{ createForm.errors.submitted_by_id }}</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Alasan Pengajuan</label><textarea v-model="createForm.reason" rows="2" placeholder="Jelaskan alasan pengajuan..." :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors resize-none bg-white dark:bg-gray-900 text-gray-900 dark:text-white', createForm.errors.reason ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']"></textarea><p v-if="createForm.errors.reason" class="text-red-500 text-xs mt-1">{{ createForm.errors.reason }}</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Bukti Pengajuan</label><input @change="createForm.attachment = $event.target.files[0]" type="file" accept=".jpg,.jpeg,.png,.pdf" :class="['w-full text-sm text-gray-500 dark:text-gray-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:bg-sky-50 file:text-sky-700 dark:file:bg-sky-900/30 dark:file:text-sky-400 hover:file:bg-sky-100 dark:hover:file:bg-sky-900/50', createForm.errors.attachment ? 'text-red-400 dark:text-red-300' : '']" /><p v-if="createForm.errors.attachment" class="text-red-500 text-xs mt-1">{{ createForm.errors.attachment }}</p></div>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showEditModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><form @submit.prevent="submitEdit" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col"><div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-edit text-sky-500 mr-2"></i>Edit Riwayat Insentif</h3><button type="button" @click="showEditModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><div class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll">
        ...
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Kode Invoice <span class="text-red-500">*</span></label><input v-model="editForm.invoice_number" type="text" placeholder="Contoh: INV/2025/001" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', editForm.errors.invoice_number ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="editForm.errors.invoice_number" class="text-red-500 text-xs mt-1">{{ editForm.errors.invoice_number }}</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Insentif <span class="text-red-500">*</span></label><SearchableSelectAjax v-model="editForm.emp_incentive_id" url="/operator-perusahaan/api/search/incentives" placeholder="— Pilih Insentif —" :error="!!editForm.errors.emp_incentive_id" :selected-label="selectedItem?.incentive_name" /><p v-if="editForm.errors.emp_incentive_id" class="text-red-500 text-xs mt-1">{{ editForm.errors.emp_incentive_id }}</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Invoice <span class="text-red-500">*</span></label><SearchableSelectAjax v-model="editForm.cust_internet_invcs_id" url="/operator-perusahaan/api/search/invoices" placeholder="— Pilih Invoice —" :error="!!editForm.errors.cust_internet_invcs_id" :selected-label="selectedItem?.invoice_number" /><p v-if="editForm.errors.cust_internet_invcs_id" class="text-red-500 text-xs mt-1">{{ editForm.errors.cust_internet_invcs_id }}</p></div>
        <div class="grid grid-cols-2 gap-3"><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Jumlah <span class="text-red-500">*</span></label><input v-model="editForm.amount" type="number" placeholder="0" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', editForm.errors.amount ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="editForm.errors.amount" class="text-red-500 text-xs mt-1">{{ editForm.errors.amount }}</p></div><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Tanggal <span class="text-red-500">*</span></label><input v-model="editForm.date" type="date" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', editForm.errors.date ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="editForm.errors.date" class="text-red-500 text-xs mt-1">{{ editForm.errors.date }}</p></div></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Diajukan Untuk</label><input v-model="editForm.submitted_by_name" type="text" placeholder="Nama orang yang diajukan" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', editForm.errors.submitted_by_name ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']" /><p v-if="editForm.errors.submitted_by_name" class="text-red-500 text-xs mt-1">{{ editForm.errors.submitted_by_name }}</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Alasan Pengajuan</label><textarea v-model="editForm.reason" rows="2" placeholder="Jelaskan alasan pengajuan..." :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors resize-none bg-white dark:bg-gray-900 text-gray-900 dark:text-white', editForm.errors.reason ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']"></textarea><p v-if="editForm.errors.reason" class="text-red-500 text-xs mt-1">{{ editForm.errors.reason }}</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Bukti Pengajuan <span class="text-xs text-gray-400">(kosongkan jika tidak diubah)</span></label><input @change="editForm.attachment = $event.target.files[0]" type="file" accept=".jpg,.jpeg,.png,.pdf" :class="['w-full text-sm text-gray-500 dark:text-gray-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:bg-sky-50 file:text-sky-700 dark:file:bg-sky-900/30 dark:file:text-sky-400 hover:file:bg-sky-100 dark:hover:file:bg-sky-900/50', editForm.errors.attachment ? 'text-red-400 dark:text-red-300' : '']" /><p v-if="editForm.errors.attachment" class="text-red-500 text-xs mt-1">{{ editForm.errors.attachment }}</p></div>
