...
function fetchData(overrides = {}) {
  router.get('/operator-saas/admin-perusahaan', buildQuery(overrides), {
    preserveState: true,
    preserveScroll: true,
    replace: true,
  });
...
function toggleSelectAll() { if (selectAll.value) selectedIds.value = adminsList.value.map(a => a.id); else selectedIds.value = []; }
function toggleSelect(id) { const i = selectedIds.value.indexOf(id); if (i === -1) selectedIds.value.push(id); else selectedIds.value.splice(i, 1); selectAll.value = selectedIds.value.length === adminsList.value.length; }
function bulkDelete() {
  if (!selectedIds.value.length) return;
  router.post('/operator-saas/admin-perusahaan/bulk-delete', { ids: [...selectedIds.value] }, {
    preserveState: true, preserveScroll: true,
    onSuccess: () => { selectedIds.value = []; selectAll.value = false; toast.success(`${selectedIds.value.length || 'Semua'} admin berhasil dihapus.`); },
    onError: () => toast.error('Gagal menghapus admin.'),
  });
}
function bulkSetStatus(status) {
  if (!selectedIds.value.length) return;
  router.post('/operator-saas/admin-perusahaan/bulk-status', { ids: [...selectedIds.value], status }, {
    preserveState: true, preserveScroll: true,
...
function restoreAdmin(admin) {
  router.post(`/operator-saas/admin-perusahaan/${admin.id}/restore`, {}, {
    preserveState: true, preserveScroll: true,
...
const createForm = useForm({ company_id: '', nama: '', email: '', kode_negara: '+62', no_telp: '', status: 'Aktif', password: '' });
const editForm = useForm({ id: null, company_id: '', nama: '', email: '', kode_negara: '+62', no_telp: '', status: 'Aktif', password: '' });
...
function openDetail(admin) { selectedAdmin.value = admin; showDetailModal.value = true; }
function openEdit(admin) {
  editForm.reset(); editForm.clearErrors();
  editForm.id = admin.id; editForm.company_id = admin.company_id;
  editForm.nama = admin.nama; editForm.email = admin.email;
  editForm.kode_negara = admin.kode_negara; editForm.no_telp = admin.no_telp;
  editForm.status = admin.status; editForm.password = '';
  if (admin.company_id && admin.perusahaan) {
    const exists = companyOptions.value.find(o => o.value === admin.company_id);
    if (!exists) companyOptions.value = [{ value: admin.company_id, label: admin.perusahaan }, ...companyOptions.value];
  }
  showEditModal.value = true;
}
function openDelete(admin) { selectedAdmin.value = admin; showDeleteModal.value = true; }
...
function confirmDelete() {
  ...
  router.delete(`/operator-saas/admin-perusahaan/${selectedAdmin.value?.id}`, {
    preserveState: true, preserveScroll: true,
    onSuccess: () => {
      showDeleteModal.value = false; toast.success(`Admin "${name}" berhasil dihapus.`);
      if (adminsList.value.length === 0 && currentPage.value > 1) goToPage(currentPage.value - 1);
    },
    onError: () => toast.error('Gagal menghapus admin.'),
  });
}
  ...
  <div>
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div><h2 class="text-2xl font-bold text-gray-900 dark:text-white">Admin Perusahaan</h2><p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Kelola akun admin level perusahaan (operator tenant).</p></div>
        <button @click="openCreate" class="inline-flex items-center px-4 py-2.5 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-colors shadow-sm" data-testid="btn-tambah"><i class="fas fa-plus mr-1.5"></i> Tambah Admin</button>
      </div>
        ...
        <div class="overflow-x-auto">
          <table data-testid="table-data" class="w-full text-sm min-w-[900px]">
              ...
              <tr>
                ...
                <th @click="sort('email')" class="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-400 cursor-pointer select-none hover:text-gray-900 dark:hover:text-white transition-colors">
                  <span class="inline-flex items-center gap-1">Email
                    <span v-if="sortOrder('email')" class="inline-flex items-center gap-0.5 text-indigo-500 dark:text-indigo-400"><i :class="['fas', sortIcon('email'), 'text-[10px]']"></i><span class="text-[10px] font-bold leading-none">{{ sortOrder('email') }}</span></span>
                    <i v-else class="fas fa-sort text-[10px] text-gray-400 dark:text-gray-500"></i>
                  </span>
                </th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-if="adminsList.length === 0"><td colspan="7" class="px-4 py-16 text-center text-gray-500 dark:text-gray-400"><i class="fas fa-user-tie text-3xl mb-2 block opacity-40"></i>Tidak ada data admin perusahaan.</td></tr>
              <tr v-for="a in adminsList" :key="a.id" :class="['transition-colors', a.dihapus ? 'bg-red-50/30 dark:bg-red-900/10 opacity-60' : 'hover:bg-gray-50 dark:hover:bg-gray-700/30']">
                <td class="px-4 py-3"><input v-if="!a.dihapus" :checked="selectedIds.includes(a.id)" type="checkbox" @change="toggleSelect(a.id)" class="rounded border-gray-300 dark:border-gray-600 text-indigo-600 focus:ring-indigo-500" /></td>
                <td class="px-4 py-3"><div class="flex items-center gap-2.5"><div class="w-8 h-8 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center text-white text-xs font-bold shrink-0">{{ a.nama.charAt(0) }}</div><span class="font-medium text-gray-900 dark:text-white whitespace-nowrap" :class="{ 'line-through': a.dihapus }">{{ a.nama }}</span></div></td>
                <td class="px-4 py-3 text-gray-600 dark:text-gray-400 whitespace-nowrap" :class="{ 'line-through': a.dihapus }">{{ a.email }}</td>
                <td class="px-4 py-3 text-gray-600 dark:text-gray-400 whitespace-nowrap max-w-[180px] truncate" :class="{ 'line-through': a.dihapus }" :title="a.perusahaan">{{ a.perusahaan || '—' }}</td>
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
          ...
          <div class="overflow-y-auto flex-1 px-6 py-5 space-y-5 modal-scroll">
            <div class="flex items-center gap-4 pb-4 border-b border-gray-100 dark:border-gray-700">
              <div class="w-16 h-16 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center text-white text-2xl font-bold shrink-0">{{ selectedAdmin?.nama?.charAt(0) }}</div>
              <div><h4 class="text-xl font-bold text-gray-900 dark:text-white">{{ selectedAdmin?.nama }}</h4><span v-if="selectedAdmin?.dihapus" class="inline-flex mt-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400">Terhapus</span><span v-else :class="['inline-flex mt-1 px-2.5 py-0.5 rounded-full text-xs font-medium', statusBadge(selectedAdmin?.status)]">{{ selectedAdmin?.status }}</span></div>
            </div>
            <div><h5 class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">Informasi Akun</h5>
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div><label class="text-xs font-medium text-gray-500 dark:text-gray-400">Perusahaan</label><p class="text-sm text-gray-900 dark:text-white mt-0.5">{{ selectedAdmin?.perusahaan || '—' }}</p></div>
            ...
            <div><h5 class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">Riwayat</h5>
              <div class="space-y-2.5">
                <div class="flex items-start gap-3"><div class="w-7 h-7 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center shrink-0 mt-0.5"><i class="fas fa-plus text-emerald-600 dark:text-emerald-400 text-xs"></i></div><div><p class="text-sm text-gray-900 dark:text-white">Dibuat</p><p class="text-xs text-gray-500 dark:text-gray-400">{{ selectedAdmin?.created_at }}</p><p v-if="selectedAdmin?.created_by" class="text-xs text-gray-400 dark:text-gray-500">oleh {{ selectedAdmin?.created_by }}</p></div></div>
                <div class="flex items-start gap-3"><div class="w-7 h-7 rounded-full bg-sky-100 dark:bg-sky-900/30 flex items-center justify-center shrink-0 mt-0.5"><i class="fas fa-pen text-sky-600 dark:text-sky-400 text-xs"></i></div><div><p class="text-sm text-gray-900 dark:text-white">Terakhir diperbarui</p><p class="text-xs text-gray-500 dark:text-gray-400">{{ selectedAdmin?.updated_at }}</p><p v-if="selectedAdmin?.updated_by" class="text-xs text-gray-400 dark:text-gray-500">oleh {{ selectedAdmin?.updated_by }}</p></div></div>
                <template v-if="selectedAdmin?.dihapus"><div class="flex items-start gap-3"><div class="w-7 h-7 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center shrink-0 mt-0.5"><i class="fas fa-trash-alt text-red-600 dark:text-red-400 text-xs"></i></div><div><p class="text-sm text-gray-900 dark:text-white">Dihapus</p><p class="text-xs text-gray-500 dark:text-gray-400">{{ selectedAdmin?.deleted_at }}</p><p v-if="selectedAdmin?.deleted_by" class="text-xs text-gray-400 dark:text-gray-500">oleh {{ selectedAdmin?.deleted_by }}</p></div></div></template>
                <template v-if="selectedAdmin?.restored_at"><div class="flex items-start gap-3"><div class="w-7 h-7 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center shrink-0 mt-0.5"><i class="fas fa-undo-alt text-emerald-600 dark:text-emerald-400 text-xs"></i></div><div><p class="text-sm text-gray-900 dark:text-white">Dipulihkan</p><p class="text-xs text-gray-500 dark:text-gray-400">{{ selectedAdmin?.restored_at }}</p><p v-if="selectedAdmin?.restored_by" class="text-xs text-gray-400 dark:text-gray-500">oleh {{ selectedAdmin?.restored_by }}</p></div></div></template>
              </div>
            </div>
          </div>
