      ...
      <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto flex-wrap">
          ...
          <div class="flex gap-1 flex-wrap">
            <button v-for="t in [{v:'',l:'Semua'},{v:'text',l:'Teks'},{v:'file',l:'File'},{v:'number',l:'Angka'},{v:'boolean',l:'Boolean'}]" :key="t.v" @click="applyTypeFilter(t.v)" :class="['px-3 py-2 rounded-lg text-xs font-medium border transition-colors whitespace-nowrap', typeFilter === t.v ? 'bg-indigo-50 border-indigo-300 text-indigo-700 dark:bg-indigo-900/30 dark:border-indigo-700 dark:text-indigo-400' : 'bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400']"><i v-if="t.v" :class="['fas mr-1', typeIcon(t.v)]"></i>{{ t.l }}</button>
          </div>
          <div class="flex gap-1 flex-wrap">
          ...
          <tr v-for="item in items" :key="item.id" @click="(e) => { if (!e.target.closest('button') && !e.target.closest('input') && !e.target.closest('a')) toggleSelect(item.id); }" :class="['border-t border-gray-100 dark:border-gray-700 cursor-pointer transition-colors', item.dihapus ? 'opacity-60' : '', 'hover:bg-gray-50 dark:hover:bg-gray-700/30']">
            ...
            <td class="px-4 py-3">
              <div class="flex items-center gap-2 max-w-xs">
                ...
                <button @click.stop="toggleReveal(item.id)" :title="revealedIds.has(item.id) ? 'Sembunyikan value' : 'Tampilkan value'" class="shrink-0 p-1 rounded text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400"><i :class="['fas text-xs', revealedIds.has(item.id) ? 'fa-eye-slash' : 'fa-eye']"></i></button>
              </div>
            </td>
