...
function fetchData(o = {}) {
  router.get('/operator-saas/konfigurasi', buildQuery(o), { preserveState: true, preserveScroll: true, replace: true });
}
...
function isMasked(item) {
  // Only kredensial is masked by default in table; other types are visible.
  if (item.type === 'kredensial') return !revealedIds.value.has(item.id);
  return false;
}
...
const activeForm = computed(() => (showCreateModal.value ? createForm : editForm));

function openCreate() { createForm.reset(); createForm.type = 'text'; kredensialReveal.value = false; showCreateModal.value = true; }
function submitCreate() { createForm.post('/operator-saas/konfigurasi', { onSuccess: () => { showCreateModal.value = false; fetchData(); toast.success('Konfigurasi berhasil ditambahkan.'); }, onError: () => toast.error('Validasi gagal: ' + errorSummary(createForm.errors), 6000) }); }
...
function submitEdit() { editForm.transform(data => ({...data, _method: 'PUT'})).post('/operator-saas/konfigurasi/' + selectedItem.value.id, { onSuccess: () => { showEditModal.value = false; fetchData(); toast.success('Konfigurasi berhasil diperbarui.'); }, onError: () => toast.error('Validasi gagal: ' + errorSummary(editForm.errors), 6000) }); }
...
function confirmDelete() { router.delete('/operator-saas/konfigurasi/' + selectedItem.value.id, { onSuccess: () => { showDeleteModal.value = false; fetchData(); toast.success('Konfigurasi berhasil dihapus.'); } }); }
function bulkDelete() { router.post('/operator-saas/konfigurasi/bulk-delete', { ids: selectedIds.value }, { onSuccess: () => { selectedIds.value = []; fetchData(); toast.success('Konfigurasi berhasil dihapus.'); } }); }
function bulkRestore() { router.post('/operator-saas/konfigurasi/bulk-restore', { ids: selectedIds.value }, { onSuccess: () => { selectedIds.value = []; fetchData(); toast.success('Konfigurasi berhasil dipulihkan.'); } }); }
function restoreItem(item) { router.post('/operator-saas/konfigurasi/' + item.id + '/restore', {}, { onSuccess: () => { fetchData(); toast.success('Konfigurasi berhasil dipulihkan.'); } }); }

function openImport() { importForm.reset(); showImportModal.value = true; }
function submitImport() { importing.value = true; importForm.post('/operator-saas/konfigurasi/import', { onSuccess: () => { showImportModal.value = false; importing.value = false; fetchData(); toast.success('Import berhasil.'); }, onError: () => { importing.value = false; toast.error('Import gagal: ' + errorSummary(importForm.errors), 6000); } }); }
function downloadTemplate() { window.location.href = '/operator-saas/konfigurasi/template'; }
...
function exportAll() {
  ...
  window.location.href = '/operator-saas/konfigurasi/export' + (params ? '?' + params : '');
}
function exportSelected() {
  if (selectedIds.value.length > 0) {
    window.location.href = '/operator-saas/konfigurasi/export?ids=' + selectedIds.value.join(',');
  } else {
    const params = buildFilterParams();
    window.location.href = '/operator-saas/konfigurasi/export' + (params ? '?' + params : '');
  }
...
<template>
  <div>
    ...
    <ToastContainer />
    <div class="space-y-6">
      <nav class="flex items-center gap-1.5 text-sm"><Link href="/operator-saas/dashboard" class="text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"><i class="fas fa-home"></i></Link><i class="fas fa-chevron-right text-[10px] text-gray-400 dark:text-gray-500"></i><span class="text-gray-900 dark:text-white font-medium">Konfigurasi</span></nav>
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div><h2 class="text-2xl font-bold text-gray-900 dark:text-white">Konfigurasi SaaS</h2><p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Kelola konfigurasi global platform SaaS (key-value).</p></div>
        <div class="flex flex-wrap items-center gap-2">
          <button v-if="can('konfigurasi.import')" @click="openImport" class="inline-flex items-center px-3 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 transition-colors shadow-sm" data-testid="btn-import"><i class="fas fa-file-import mr-1.5"></i> Import</button>
          <button v-if="can('konfigurasi.import')" @click="downloadTemplate" class="inline-flex items-center px-3 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors shadow-sm"><i class="fas fa-download mr-1.5"></i> Template</button>
          <div v-if="can('konfigurasi.export')" class="relative group">
            <button class="inline-flex items-center px-3 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-colors shadow-sm"><i class="fas fa-file-export mr-1.5"></i> Export <i class="fas fa-chevron-down ml-1.5 text-xs"></i></button>
          ...
          <button v-if="can('konfigurasi.create') && !isTrashedView" @click="openCreate" class="inline-flex items-center px-4 py-2.5 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-colors shadow-sm" data-testid="btn-tambah"><i class="fas fa-plus mr-1.5"></i> Tambah</button>
        </div>
      </div>
      <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto flex-wrap">
          <div class="relative w-full sm:w-72"><div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><i class="fas fa-search text-gray-400 text-sm"></i></div><input v-model="searchInput" type="text" placeholder="Cari key, value, atau deskripsi..." class="w-full pl-10 pr-16 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" @keydown.enter="applySearch" data-testid="input-search"><div class="absolute inset-y-0 right-0 flex items-center gap-1 pr-1.5"><button v-if="searchInput" @click="clearSearch" class="p-1 rounded text-gray-400 hover:text-gray-600 dark:hover:text-white" title="Clear"><i class="fas fa-times text-xs"></i></button><button @click="applySearch" class="px-2 py-1 rounded bg-indigo-600 text-white hover:bg-indigo-700" title="Cari"><i class="fas fa-search text-xs"></i></button></div></div>
          <div class="flex gap-1 flex-wrap">
            <button v-for="t in [{v:'',l:'Semua'},{v:'text',l:'Teks'},{v:'file',l:'File'},{v:'number',l:'Angka'},{v:'boolean',l:'Boolean'},{v:'kredensial',l:'Kredensial'}]" :key="t.v" @click="applyTypeFilter(t.v)" :class="['px-3 py-2 rounded-lg text-xs font-medium border transition-colors whitespace-nowrap', typeFilter === t.v ? 'bg-indigo-50 border-indigo-300 text-indigo-700 dark:bg-indigo-900/30 dark:border-indigo-700 dark:text-indigo-400' : 'bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400']"><i v-if="t.v" :class="['fas mr-1', typeIcon(t.v)]"></i>{{ t.l }}</button>
          </div>
          <div class="flex gap-1 flex-wrap">
            <button v-for="t in [{v:'tidak',l:'Aktif'},{v:'ya',l:'Terhapus'}]" :key="t.v" @click="applyTerhapusFilter(t.v)" :class="['px-3 py-2 rounded-lg text-xs font-medium border transition-colors whitespace-nowrap', terhapusFilter === t.v ? (t.v === 'ya' ? 'bg-red-50 border-red-300 text-red-700 dark:bg-red-900/30 dark:border-red-700 dark:text-red-400' : 'bg-indigo-50 border-indigo-300 text-indigo-700 dark:bg-indigo-900/30 dark:border-indigo-700 dark:text-indigo-400') : 'bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400']"><i :class="['fas mr-1', t.v === 'ya' ? 'fa-trash-alt' : 'fa-check-circle']"></i>{{ t.l }}</button>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
        ...
        <tbody>
          ...
          <tr v-for="item in items" :key="item.id" @click="(e) => { if (!e.target.closest('button') && !e.target.closest('input') && !e.target.closest('a')) selectedIds = selectedIds.includes(item.id) ? selectedIds.filter(x => x !== item.id) : [...selectedIds, item.id]; }" :class="['border-t border-gray-100 dark:border-gray-700 cursor-pointer transition-colors', item.dihapus ? 'opacity-60' : '', 'hover:bg-gray-50 dark:hover:bg-gray-700/30']">
            <td class="px-4 py-3" @click.stop><input v-model="selectedIds" :value="item.id" type="checkbox" class="rounded border-gray-300 text-indigo-600" /></td>
            ...
            <td class="px-4 py-3">
              <div class="flex items-center gap-2 max-w-xs">
                ...
                <button v-if="item.type === 'kredensial'" @click.stop="toggleReveal(item.id)" :title="revealedIds.has(item.id) ? 'Sembunyikan value' : 'Tampilkan value'" class="shrink-0 p-1 rounded text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400"><i :class="['fas text-xs', revealedIds.has(item.id) ? 'fa-eye-slash' : 'fa-eye']"></i></button>
              </div>
            </td>
            ...
            <td class="px-4 py-3" @click.stop>
              <div class="flex items-center justify-center gap-1">
                <button v-if="can('konfigurasi.list')" @click="openDetail(item)" title="Detail" class="p-1.5 rounded-lg text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:text-indigo-400 dark:hover:bg-indigo-900/30"><i class="fas fa-eye"></i></button>
                <button v-if="can('konfigurasi.edit') && !item.dihapus" @click="openEdit(item)" title="Edit" class="p-1.5 rounded-lg text-gray-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:text-amber-400 dark:hover:bg-amber-900/30"><i class="fas fa-edit"></i></button>
                <button v-if="can('konfigurasi.restore') && item.dihapus" @click="restoreItem(item)" title="Pulihkan" class="p-1.5 rounded-lg text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:text-emerald-400 dark:hover:bg-emerald-900/30"><i class="fas fa-undo-alt"></i></button>
                <button v-if="can('konfigurasi.delete') && !item.dihapus" @click="openDelete(item)" title="Hapus" class="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:text-red-400 dark:hover:bg-red-900/30"><i class="fas fa-trash-alt"></i></button>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showDetailModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showDetailModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col"><div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-eye mr-2 text-indigo-500"></i>Detail Konfigurasi</h3><button @click="showDetailModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><div class="overflow-y-auto flex-1 px-6 py-5 space-y-5 modal-scroll" v-if="selectedItem">
        ...
        <div v-if="selectedItem.dihapus" class="grid grid-cols-2 gap-3 border-t border-gray-200 dark:border-gray-700 pt-3">
          <div><label class="text-xs font-medium text-gray-500 dark:text-gray-400">Dihapus Pada</label><p class="text-sm text-gray-700 dark:text-gray-300">{{ selectedItem.deleted_at || '-' }}</p></div>
          <div><label class="text-xs font-medium text-gray-500 dark:text-gray-400">Dihapus Oleh</label><p class="text-sm text-gray-700 dark:text-gray-300">{{ selectedItem.deleted_by || '-' }}</p></div>
        </div>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showCreateModal || showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showCreateModal = showEditModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col"><div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i :class="['fas mr-2', showCreateModal ? 'fa-plus text-emerald-500' : 'fa-edit text-amber-500']"></i>{{ showCreateModal ? 'Tambah Konfigurasi' : 'Edit Konfigurasi' }}</h3><button @click="showCreateModal = showEditModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><form class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll" @submit.prevent="showCreateModal ? submitCreate() : submitEdit()">
        <FormErrorSummary :errors="activeForm.errors" testId="form-error-summary-konfigurasi" />
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Key <span class="text-red-500">*</span></label><input v-model="activeForm.key" type="text" placeholder="Contoh: app.tagline" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm font-mono focus:ring-2 outline-none', activeForm.errors.key ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-indigo-500']" /><p v-if="activeForm.errors.key" class="text-red-500 text-xs mt-1">{{ activeForm.errors.key }}</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Tipe <span class="text-red-500">*</span></label><select v-model="activeForm.type" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm', activeForm.errors.type ? 'border-red-400' : 'border-gray-300 dark:border-gray-600']"><option value="text">Teks</option><option value="file">File</option><option value="number">Angka</option><option value="boolean">Boolean (true/false)</option><option value="kredensial">Kredensial (disembunyikan default)</option></select><p v-if="activeForm.errors.type" class="text-red-500 text-xs mt-1">{{ activeForm.errors.type }}</p></div>
        <div>
          <div class="flex items-center justify-between mb-1.5">
            ...
            <button v-if="activeForm.type === 'kredensial'" type="button" @click="kredensialReveal = !kredensialReveal" :title="kredensialReveal ? 'Sembunyikan value' : 'Tampilkan value'" class="p-1.5 rounded text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400"><i :class="['fas text-xs', kredensialReveal ? 'fa-eye-slash' : 'fa-eye']"></i></button>
          </div>
          <textarea v-if="activeForm.type === 'text' || activeForm.type === 'file'" v-model="activeForm.value" rows="4" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm font-mono focus:ring-2 outline-none resize-none', activeForm.errors.value ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-indigo-500']" :placeholder="activeForm.type === 'file' ? 'Path file atau URL' : 'Teks bebas'"></textarea>
          <input v-else-if="activeForm.type === 'number'" :value="activeForm.value" @input="activeForm.value = String($event.target.value)" type="number" step="any" placeholder="0" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm font-mono focus:ring-2 outline-none', activeForm.errors.value ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-indigo-500']" />
          <select v-else-if="activeForm.type === 'boolean'" :value="activeForm.value" @change="activeForm.value = String($event.target.value)" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm', activeForm.errors.value ? 'border-red-400' : 'border-gray-300 dark:border-gray-600']"><option value="">-- pilih --</option><option value="true">true (Ya)</option><option value="false">false (Tidak)</option></select>
          <input v-else-if="activeForm.type === 'kredensial'" v-model="activeForm.value" :type="kredensialReveal ? 'text' : 'password'" autocomplete="off" :placeholder="kredensialReveal ? 'API key / token / secret (visible)' : 'API key / token / secret (masked)'" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm font-mono focus:ring-2 outline-none', activeForm.errors.value ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-rose-500']" />
          <p v-if="activeForm.errors.value" class="text-red-500 text-xs mt-1">{{ activeForm.errors.value }}</p>
        </div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Description</label><textarea v-model="activeForm.description" rows="2" placeholder="Penjelasan singkat (opsional)" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 outline-none resize-none', activeForm.errors.description ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-indigo-500']"></textarea><p v-if="activeForm.errors.description" class="text-red-500 text-xs mt-1">{{ activeForm.errors.description }}</p></div>
        <div class="shrink-0 flex justify-end gap-2 pt-2 border-t border-gray-200 dark:border-gray-700 -mx-6 px-6 -mb-5 pb-4"><button type="button" @click="showCreateModal = showEditModal = false" class="px-4 py-2.5 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">Batal</button><button type="submit" class="px-6 py-2.5 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-colors shadow-sm"><i class="fas fa-save mr-1.5"></i>{{ showCreateModal ? 'Simpan' : 'Update' }}</button></div>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showImportModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showImportModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><form @submit.prevent="submitImport" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col"><div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-file-import text-emerald-500 mr-2"></i>Import Konfigurasi</h3><button type="button" @click="showImportModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><div class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll">
        <FormErrorSummary :errors="importForm.errors" testId="form-error-summary-import-konfigurasi" />
        ...
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">File Excel <span class="text-red-500">*</span></label><input @change="importForm.file = $event.target.files[0]" type="file" accept=".xlsx,.csv" class="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-indigo-50 file:text-indigo-700 dark:file:bg-indigo-900/30 dark:file:text-indigo-400 hover:file:bg-indigo-100 dark:hover:file:bg-indigo-900/50" /><p v-if="importForm.errors.file" class="text-red-500 text-xs mt-1">{{ importForm.errors.file }}</p></div>
        <button type="button" @click="downloadTemplate" class="text-xs text-indigo-600 dark:text-indigo-400 hover:underline"><i class="fas fa-download mr-1"></i>Download template</button>
