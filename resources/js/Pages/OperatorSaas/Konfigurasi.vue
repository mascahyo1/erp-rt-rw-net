...
function submitCreate() { createForm.post('/operator-saas/konfigurasi', { onSuccess: () => { showCreateModal.value = false; fetchData(); toast.success('Konfigurasi berhasil ditambahkan.'); } }); }
...
function submitEdit() { editForm.put('/operator-saas/konfigurasi/' + selectedItem.value.id, { onSuccess: () => { showEditModal.value = false; fetchData(); toast.success('Konfigurasi berhasil diperbarui.'); } }); }
...
function bulkDelete() { router.post('/operator-saas/konfigurasi/bulk-delete', { ids: selectedIds.value }, { onSuccess: () => { selectedIds.value = []; selectAll.value = false; fetchData(); toast.success('Konfigurasi berhasil dihapus.'); } }); }
function bulkRestore() { router.post('/operator-saas/konfigurasi/bulk-restore', { ids: selectedIds.value }, { onSuccess: () => { selectedIds.value = []; selectAll.value = false; fetchData(); toast.success('Konfigurasi berhasil dipulihkan.'); } }); }
...
function submitImport() { importing.value = true; importForm.post('/operator-saas/konfigurasi/import', { onSuccess: () => { showImportModal.value = false; importing.value = false; fetchData(); toast.success('Import berhasil.'); }, onError: () => { importing.value = false; toast.error('Import gagal.'); } }); }
...
<template>
  <div>
      ...
      <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
        ...
        <tbody>
          ...
          <tr v-for="item in items" :key="item.id" @click="(e) => { if (!e.target.closest('button') && !e.target.closest('input') && !e.target.closest('a')) toggleSelect(item.id); }" :class="['border-t border-gray-100 dark:border-gray-700 cursor-pointer transition-colors', item.dihapus ? 'opacity-60' : '', 'hover:bg-gray-50 dark:hover:bg-gray-700/30']">
            <td class="px-4 py-3" @click.stop><input v-model="selectedIds" :value="item.id" type="checkbox" @change="toggleSelect(item.id)" class="rounded border-gray-300 text-indigo-600" /></td>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showCreateModal || showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showCreateModal = showEditModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col"><div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i :class="['fas mr-2', showCreateModal ? 'fa-plus text-emerald-500' : 'fa-edit text-amber-500']"></i>{{ showCreateModal ? 'Tambah Konfigurasi' : 'Edit Konfigurasi' }}</h3><button @click="showCreateModal = showEditModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><form class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll" @submit.prevent="showCreateModal ? submitCreate() : submitEdit()">
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Key <span class="text-red-500">*</span></label><input v-model="(showCreateModal ? createForm : editForm).key" type="text" placeholder="Contoh: app.tagline" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm font-mono focus:ring-2 focus:ring-indigo-500 outline-none" /><p v-if="(showCreateModal ? createForm : editForm).errors.key" class="text-red-500 text-xs mt-1">{{ (showCreateModal ? createForm : editForm).errors.key }}</p></div>
        ...
        <div>
          <div class="flex items-center justify-between mb-1.5">
            ...
            <button v-if="(showCreateModal ? createForm : editForm).type === 'kredensial'" type="button" @click="showCreateModal ? (createValueVisible = !createValueVisible) : (editValueVisible = !editValueVisible)" :title="(showCreateModal ? createValueVisible : editValueVisible) ? 'Sembunyikan value' : 'Tampilkan value'" class="p-1.5 rounded text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400"><i :class="['fas text-xs', (showCreateModal ? createValueVisible : editValueVisible) ? 'fa-eye-slash' : 'fa-eye']"></i></button>
          </div>
          <template v-if="valueVisible">
            <textarea v-if="(showCreateModal ? createForm : editForm).type === 'text' || (showCreateModal ? createForm : editForm).type === 'file'" v-model="(showCreateModal ? createForm : editForm).value" rows="4" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm font-mono focus:ring-2 focus:ring-indigo-500 outline-none resize-none" :placeholder="(showCreateModal ? createForm : editForm).type === 'file' ? 'Path file atau URL' : 'Teks bebas'"></textarea>
            <input v-else-if="(showCreateModal ? createForm : editForm).type === 'number'" :value="(showCreateModal ? createForm : editForm).value" @input="(showCreateModal ? createForm : editForm).value = String($event.target.value)" type="number" step="any" placeholder="0" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm font-mono focus:ring-2 focus:ring-indigo-500 outline-none" />
            <select v-else-if="(showCreateModal ? createForm : editForm).type === 'boolean'" :value="(showCreateModal ? createForm : editForm).value" @change="(showCreateModal ? createForm : editForm).value = String($event.target.value)" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm"><option value="">-- pilih --</option><option value="true">true (Ya)</option><option value="false">false (Tidak)</option></select>
        ...
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Description</label><textarea v-model="(showCreateModal ? createForm : editForm).description" rows="2" placeholder="Penjelasan singkat (opsional)" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500 outline-none resize-none"></textarea></div>
