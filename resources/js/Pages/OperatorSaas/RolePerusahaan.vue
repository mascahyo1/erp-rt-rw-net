...
const permissionGroups = [
  {
    module: 'Perusahaan Saya',
    permissions: [
      { key: 'perusahaan-saya.detail', label: 'Lihat Detail' },
      { key: 'perusahaan-saya.ubah', label: 'Ubah Data' },
    ]
  },
  {
    module: 'Daftar Paket',
    permissions: [
      { key: 'daftar-paket.list', label: 'Lihat Daftar' },
      { key: 'daftar-paket.tambah', label: 'Tambah' },
      { key: 'daftar-paket.ubah', label: 'Ubah' },
      { key: 'daftar-paket.detail', label: 'Detail' },
      { key: 'daftar-paket.hapus', label: 'Hapus' },
      { key: 'daftar-paket.import', label: 'Import' },
      { key: 'daftar-paket.export', label: 'Export' },
    ]
  },
  {
    module: 'Pelanggan',
    permissions: [
      { key: 'pelanggan.list', label: 'Lihat Daftar' },
      { key: 'pelanggan.tambah', label: 'Tambah' },
      { key: 'pelanggan.ubah', label: 'Ubah' },
      { key: 'pelanggan.detail', label: 'Detail' },
      { key: 'pelanggan.hapus', label: 'Hapus' },
      { key: 'pelanggan.import', label: 'Import' },
      { key: 'pelanggan.export', label: 'Export' },
    ]
  },
  {
    module: 'Paket Pelanggan',
    permissions: [
      { key: 'paket-pelanggan.list', label: 'Lihat Daftar' },
      { key: 'paket-pelanggan.tambah', label: 'Tambah' },
      { key: 'paket-pelanggan.ubah', label: 'Ubah' },
      { key: 'paket-pelanggan.detail', label: 'Detail' },
      { key: 'paket-pelanggan.hapus', label: 'Hapus' },
      { key: 'paket-pelanggan.import', label: 'Import' },
      { key: 'paket-pelanggan.export', label: 'Export' },
    ]
  },
  {
    module: 'Pembayaran Pelanggan',
    permissions: [
      { key: 'pembayaran-pelanggan.list', label: 'Lihat Daftar' },
      { key: 'pembayaran-pelanggan.tambah', label: 'Tambah' },
      { key: 'pembayaran-pelanggan.ubah', label: 'Ubah' },
      { key: 'pembayaran-pelanggan.detail', label: 'Detail' },
      { key: 'pembayaran-pelanggan.hapus', label: 'Hapus' },
      { key: 'pembayaran-pelanggan.import', label: 'Import' },
      { key: 'pembayaran-pelanggan.export', label: 'Export' },
    ]
  },
  {
    module: 'Konfigurasi Perusahaan',
    permissions: [
      { key: 'konfigurasi-perusahaan.list', label: 'Lihat Daftar' },
      { key: 'konfigurasi-perusahaan.tambah', label: 'Tambah' },
      { key: 'konfigurasi-perusahaan.ubah', label: 'Ubah' },
      { key: 'konfigurasi-perusahaan.detail', label: 'Detail' },
      { key: 'konfigurasi-perusahaan.hapus', label: 'Hapus' },
      { key: 'konfigurasi-perusahaan.import', label: 'Import' },
      { key: 'konfigurasi-perusahaan.export', label: 'Export' },
    ]
  },
];
...
// ── Data ──
const roles = ref([
  { id: 1, nama_role: 'Super Admin', perusahaan: 'PT Net Sejahtera', deskripsi: 'Akses penuh ke semua fitur perusahaan', status: 'Aktif', permissions: [...allPermissionKeys.value], created_at: '2025-01-10' },
  { id: 2, nama_role: 'Admin Keuangan', perusahaan: 'PT Net Sejahtera', deskripsi: 'Mengelola tagihan dan pembayaran', status: 'Aktif', permissions: ['perusahaan-saya.detail','daftar-paket.list','daftar-paket.tambah','daftar-paket.ubah','daftar-paket.detail','pelanggan.list','pelanggan.tambah','pelanggan.ubah','pelanggan.detail','paket-pelanggan.list','pembayaran-pelanggan.list','pembayaran-pelanggan.tambah','pembayaran-pelanggan.ubah','pembayaran-pelanggan.detail','konfigurasi-perusahaan.list'], created_at: '2025-01-12' },
  { id: 3, nama_role: 'Kolektor', perusahaan: 'CV Digital Media', deskripsi: 'Input pembayaran & tagih pelanggan', status: 'Aktif', permissions: ['pelanggan.list','pelanggan.detail','pembayaran-pelanggan.list','pembayaran-pelanggan.tambah','pembayaran-pelanggan.detail'], created_at: '2025-02-18' },
  { id: 4, nama_role: 'Viewer', perusahaan: 'UD Net Mandiri', deskripsi: 'Hanya melihat laporan', status: 'Nonaktif', permissions: ['daftar-paket.list','pelanggan.list','paket-pelanggan.list','pembayaran-pelanggan.list','konfigurasi-perusahaan.list'], created_at: '2025-03-05' },
  { id: 5, nama_role: 'Admin', perusahaan: 'PT Teknologi Nusantara', deskripsi: 'Kelola pelanggan dan karyawan', status: 'Aktif', permissions: ['perusahaan-saya.detail','perusahaan-saya.ubah','daftar-paket.list','daftar-paket.tambah','daftar-paket.ubah','daftar-paket.detail','pelanggan.list','pelanggan.tambah','pelanggan.ubah','pelanggan.detail','pelanggan.hapus','paket-pelanggan.list','paket-pelanggan.tambah','paket-pelanggan.ubah','paket-pelanggan.detail','pembayaran-pelanggan.list','pembayaran-pelanggan.tambah','pembayaran-pelanggan.ubah','pembayaran-pelanggan.detail','konfigurasi-perusahaan.list','konfigurasi-perusahaan.ubah'], created_at: '2025-03-22' },
]);
...
const form = ref({ id: null, nama_role: '', perusahaan: '', deskripsi: '', status: 'Aktif', permissions: [] });
...
function applyPerusahaanFilter(p) { perusahaanFilter.value = p; currentPage.value = 1; }
...
const filteredRoles = computed(() => {
  let r = [...roles.value];
  if (search.value) { const q = search.value.toLowerCase(); r = r.filter(x => x.nama_role.toLowerCase().includes(q) || x.perusahaan.toLowerCase().includes(q)); }
...
function isGroupPartialChecked(group) { const checked = group.permissions.filter(p => isPermChecked(p.key)).length; return checked > 0 && checked < group.permissions.length; }
function permissionCount(r) { return r?.permissions?.length || 0; }

function openCreate() { form.value = { id: null, nama_role: '', perusahaan: '', deskripsi: '', status: 'Aktif', permissions: [] }; formErrors.value = {}; showCreateModal.value = true; }
...
function validateForm() { const e = {}; if (!form.value.nama_role.trim()) e.nama_role = 'Nama role wajib diisi'; if (!form.value.perusahaan) e.perusahaan = 'Perusahaan wajib dipilih'; formErrors.value = e; return Object.keys(e).length === 0; }
  ...
  <div>
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400"><span>{{ filteredRoles.length }} data</span><button v-if="search || statusFilter || perusahaanFilter" @click="searchInput = ''; search = ''; applyStatusFilter(''); perusahaanFilter = ''" class="text-xs text-red-500 hover:text-red-700 dark:hover:text-red-400 underline">Reset filter</button></div>
      </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm min-w-[700px]">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-if="paginatedRoles.length === 0"><td colspan="6" class="px-4 py-16 text-center text-gray-500 dark:text-gray-400"><i class="fas fa-tags text-3xl mb-2 block opacity-40"></i>Tidak ada data role perusahaan.</td></tr>
              <tr v-for="r in paginatedRoles" :key="r.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                ...
                <td class="px-4 py-3 text-gray-600 dark:text-gray-400 whitespace-nowrap">{{ r.perusahaan }}</td>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showDetailModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showDetailModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto"><div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-eye mr-2 text-indigo-500"></i>Detail Role</h3><button @click="showDetailModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><div class="px-6 py-5 space-y-4"><div class="flex items-center gap-4 pb-4 border-b border-gray-100 dark:border-gray-700"><div class="w-16 h-16 rounded-lg bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center text-white text-xl shrink-0"><i class="fas fa-tag"></i></div><div><h4 class="text-xl font-bold text-gray-900 dark:text-white">{{ selectedRole?.nama_role }}</h4><span :class="['inline-flex mt-1 px-2.5 py-0.5 rounded-full text-xs font-medium', statusBadge(selectedRole?.status)]">{{ selectedRole?.status }}</span></div></div><div class="grid grid-cols-1 sm:grid-cols-2 gap-4"><div><label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Perusahaan</label><p class="text-sm text-gray-900 dark:text-white mt-1">{{ selectedRole?.perusahaan }}</p></div><div><label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Tanggal Dibuat</label><p class="text-sm text-gray-900 dark:text-white mt-1">{{ selectedRole?.created_at }}</p></div></div><div><label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Deskripsi</label><p class="text-sm text-gray-900 dark:text-white mt-1">{{ selectedRole?.deskripsi || '—' }}</p></div><div><label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2 block">Permission ({{ permissionCount(selectedRole) }} akses)</label><div class="grid grid-cols-1 md:grid-cols-2 gap-3"><div v-for="group in permissionGroups" :key="group.module" class="rounded-lg border border-gray-200 dark:border-gray-700 p-3"><h5 class="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-2">{{ group.module }}</h5><div class="flex flex-wrap gap-1"><span v-for="p in group.permissions" :key="p.key" :class="['inline-flex px-2 py-0.5 rounded text-[11px] font-medium', selectedRole?.permissions?.includes(p.key) ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-gray-100 text-gray-400 dark:bg-gray-700 dark:text-gray-500']">{{ p.label }}</span></div></div></div></div></div><div class="flex justify-end gap-2 px-6 py-4 bg-gray-50 dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700"><button @click="showDetailModal = false; openEdit(selectedRole)" class="px-4 py-2 bg-sky-600 text-white text-sm font-medium rounded-lg hover:bg-sky-700 transition-colors"><i class="fas fa-edit mr-1.5"></i> Edit</button><button @click="showDetailModal = false" class="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">Tutup</button></div></div></div>
      </Transition>
    </Teleport>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showCreateModal || showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showCreateModal = false; showEditModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto"><div class="sticky top-0 z-10 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-t-2xl"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i :class="['fas mr-2', showCreateModal ? 'fa-plus text-emerald-500' : 'fa-edit text-sky-500']"></i>{{ showCreateModal ? 'Tambah Role Perusahaan' : 'Edit Role Perusahaan' }}</h3><button @click="showCreateModal = false; showEditModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><form class="px-6 py-5 space-y-4" @submit.prevent="showCreateModal ? saveCreate() : saveEdit()">
          <div>
            ...
            <input v-model="form.nama_role" type="text" placeholder="Contoh: Admin Keuangan" :class="['w-full px-3 py-2.5 rounded-lg border text-sm outline-none transition-colors bg-white dark:bg-gray-900 text-gray-900 dark:text-white', formErrors.nama_role ? 'border-red-400 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500']" />
            <p v-if="formErrors.nama_role" class="text-red-500 text-xs mt-1">{{ formErrors.nama_role }}</p>
          </div>
          <div>
            ...
            <SearchableSelect v-model="form.perusahaan" :options="perusahaanOptions" placeholder="— Pilih Perusahaan —" />
            <p v-if="formErrors.perusahaan" class="text-red-500 text-xs mt-1">{{ formErrors.perusahaan }}</p>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Deskripsi</label>
            <textarea v-model="form.deskripsi" rows="2" placeholder="Deskripsi role (opsional)" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-colors resize-none"></textarea>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Status</label>
            <select v-model="form.status" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-colors"><option value="Aktif">Aktif</option><option value="Nonaktif">Nonaktif</option></select>
          </div>
          <!-- Permission Section -->
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Permission <span class="text-xs text-gray-400 font-normal ml-1">({{ form.permissions.length }} dipilih)</span></label>
          ...
          <div class="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" class="px-6 py-2.5 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition-colors shadow-sm"><i :class="['fas mr-1.5', showCreateModal ? 'fa-save' : 'fa-check']"></i>{{ showCreateModal ? 'Simpan' : 'Update' }}</button>
          </div>
        </form></div></div>
      </Transition>
    </Teleport>
