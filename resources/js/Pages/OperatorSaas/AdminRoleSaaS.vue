...
const adminList = ['Hendra Wijaya', 'Rina Marlina', 'Doni Prasetyo', 'Fitri Handayani', 'Agus Kurniawan'];
const roleList = ['Super Admin SaaS', 'Admin Support', 'Operator', 'Auditor'];

const adminOptions = computed(() => adminList.map(a => ({ value: a, label: a })));
const roleOptions = computed(() => roleList.map(r => ({ value: r, label: r })));

const mappings = ref([
  { id: 1, admin: 'Hendra Wijaya', role: 'Super Admin SaaS', status: 'Aktif', created_at: '2025-01-05' },
  { id: 2, admin: 'Rina Marlina', role: 'Admin Support', status: 'Aktif', created_at: '2025-02-10' },
  { id: 3, admin: 'Doni Prasetyo', role: 'Operator', status: 'Aktif', created_at: '2025-03-15' },
  { id: 4, admin: 'Fitri Handayani', role: 'Auditor', status: 'Nonaktif', created_at: '2025-04-20' },
  { id: 5, admin: 'Agus Kurniawan', role: 'Admin Support', status: 'Aktif', created_at: '2025-05-10' },
]);
...
const form = ref({ id: null, admin: '', role: '', status: 'Aktif' });
...
const filteredMappings = computed(() => {
  ...
  if (search.value) { const q = search.value.toLowerCase(); r = r.filter(x => x.admin.toLowerCase().includes(q) || x.role.toLowerCase().includes(q)); }
...
<template>
  <div>
      ...
      <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          <div class="relative w-full sm:w-72">
            ...
            <input v-model="searchInput" type="text" placeholder="Cari admin atau role..." class="w-full pl-10 pr-16 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-colors" @keydown.enter="applySearch" />
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-sm min-w-[600px]">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-if="paginatedMappings.length === 0"><td colspan="5" class="px-4 py-16 text-center text-gray-500 dark:text-gray-400"><i class="fas fa-users-cog text-3xl mb-2 block opacity-40"></i>Tidak ada data admin role SaaS.</td></tr>
              <tr v-for="m in paginatedMappings" :key="m.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                ...
                <td class="px-4 py-3"><div class="flex items-center gap-2.5"><div class="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-pink-600 flex items-center justify-center text-white text-xs font-bold shrink-0">{{ m.admin.charAt(0) }}</div><span class="font-medium text-gray-900 dark:text-white whitespace-nowrap">{{ m.admin }}</span></div></td>
                <td class="px-4 py-3 text-gray-600 dark:text-gray-400 whitespace-nowrap"><span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"><i class="fas fa-user-tag text-[10px]"></i> {{ m.role }}</span></td>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showCreateModal || showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showCreateModal = false; showEditModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto"><div class="sticky top-0 z-10 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-t-2xl"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i :class="['fas mr-2', showCreateModal ? 'fa-plus text-emerald-500' : 'fa-edit text-sky-500']"></i>{{ showCreateModal ? 'Tambah Admin Role' : 'Edit Admin Role' }}</h3><button @click="showCreateModal = false; showEditModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><form class="px-6 py-5 space-y-4" @submit.prevent="showCreateModal ? saveCreate() : saveEdit()">
      ...
      <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Role <span class="text-red-500">*</span></label><SearchableSelect v-model="form.role" :options="roleOptions" placeholder="— Pilih Role —" /><p v-if="formErrors.role" class="text-red-500 text-xs mt-1">{{ formErrors.role }}</p></div>
