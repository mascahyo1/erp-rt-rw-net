...
function openCreate() { createForm.reset(); createForm.clearErrors(); createForm.provider = 'internal'; createForm.payment_method = 'tunai'; createFormFile.value = null; showCreateModal.value = true; }
async function submitCreate() {
  ...
  if (createForm.provider === 'midtrans') {
    ...
    try {
      ...
      const resp = await fetch('/karyawan/riwayat-pembayaran', {
        method: 'POST',
      ...
      if (resp.ok && data.redirect_url) {
        ...
        // Open Snap UI di tab baru — karyawan bisa bantu customer selesaikan pembayaran
        window.open(data.redirect_url, '_blank', 'noopener,noreferrer');
        fetchData();
      } else {
        const errs = data.errors || {};
        if (Object.keys(errs).length > 0) {
          createForm.setError(errs);
  ...
  router.post('/karyawan/riwayat-pembayaran', formData, {
    preserveState: true, preserveScroll: true,
    onSuccess: () => { showCreateModal.value = false; fetchData(); toast.success('Pembayaran berhasil ditambahkan.'); },
    onError: (errors) => { createForm.setError(errors || {}); toast.error('Validasi gagal: ' + errorSummary(errors || createForm.errors), 6000); }
  });
}
...
function submitEdit() {
  ...
  router.post('/karyawan/riwayat-pembayaran/' + selectedItem.value.id + '?_method=PUT', formData, {
    preserveState: true, preserveScroll: true,
    onSuccess: () => { showEditModal.value = false; fetchData(); toast.success('Pembayaran berhasil diperbarui.'); },
    onError: (errors) => { editForm.setError(errors || {}); toast.error('Validasi gagal: ' + errorSummary(errors || editForm.errors), 6000); }
  });
}
...
function submitReview() {
  reviewForm.post('/karyawan/riwayat-pembayaran/' + selectedItem.value.id + '/review', {
    preserveState: true, preserveScroll: true,
...
function submitBulkReview() {
  bulkReviewForm.post('/karyawan/riwayat-pembayaran/bulk-review', {
    preserveState: true, preserveScroll: true,
...
function bulkDelete() { router.post('/karyawan/riwayat-pembayaran/bulk-delete', { ids: selectedIds.value }, { onSuccess: () => { selectedIds.value = []; fetchData(); toast.success('Pembayaran berhasil dihapus.'); } }); }
function bulkRestore() { router.post('/karyawan/riwayat-pembayaran/bulk-restore', { ids: selectedIds.value }, { onSuccess: () => { selectedIds.value = []; fetchData(); toast.success('Pembayaran berhasil dipulihkan.'); } }); }
async function bulkVerifyMidtrans() {
  ...
  try {
    ...
    const res = await fetch('/karyawan/api/riwayat-pembayaran/bulk-verify-midtrans', {
      method: 'POST',
...
function downloadTemplate() { window.location.href = '/karyawan/riwayat-pembayaran/template'; }
function openImport() { importFile.value = null; importForm.clearErrors(); showImportModal.value = true; }
...
function submitImport() {
  ...
  router.post('/karyawan/riwayat-pembayaran/import', formData, {
    onSuccess: () => { showImportModal.value = false; importing.value = false; fetchData(); toast.success('Import berhasil.'); },
    onError: (errors) => { importing.value = false; importForm.setError(errors || {}); toast.error('Import gagal: ' + errorSummary(errors || importForm.errors), 6000); }
  });
}
...
async function verifyMidtrans(item) {
  ...
  try {
    const csrf = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '';
    const res = await fetch(`/karyawan/api/riwayat-pembayaran/${item.id}/verify-midtrans`, {
      method: 'POST',
...
function exportAll() {
  ...
  window.open('/karyawan/riwayat-pembayaran/export?' + params, '_blank');
...
function exportSelected() {
  ...
  window.open('/karyawan/riwayat-pembayaran/export?' + params.toString(), '_blank');
  showExportDropdown.value = false;
}
  ...
  <div>
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div><h2 class="text-2xl font-bold text-gray-900 dark:text-white">Riwayat Pembayaran</h2><p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Riwayat pembayaran yang Anda collection dari pelanggan.</p></div>
        <div class="flex items-center gap-2">
          <button v-if="can('karyawan-riwayat-pembayaran.import') && terhapusFilter !== 'ya'" @click="openImport" class="inline-flex items-center px-3 py-2 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 transition-colors shadow-sm" data-testid="btn-import"><i class="fas fa-file-import mr-1.5"></i> Import</button>
          <button v-if="can('karyawan-riwayat-pembayaran.import')" @click="downloadTemplate" class="inline-flex items-center px-3 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors shadow-sm"><i class="fas fa-download mr-1.5"></i> Template</button>
          <div v-if="can('karyawan-riwayat-pembayaran.export')" class="relative">
            <button @click="showExportDropdown = !showExportDropdown" class="inline-flex items-center px-3 py-2 bg-amber-600 text-white text-sm font-medium rounded-lg hover:bg-amber-700 transition-colors shadow-sm">
            ...
            <div v-if="showExportDropdown" class="absolute right-0 mt-1 w-44 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-lg py-1 z-30">
              <button @click="exportAll" class="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700" data-testid="btn-export"><i class="fas fa-list mr-2 text-emerald-500"></i>Export Semua</button>
          ...
          <button v-if="can('karyawan-riwayat-pembayaran.create') && terhapusFilter !== 'ya'" @click="openCreate" class="inline-flex items-center px-4 py-2.5 bg-amber-600 text-white text-sm font-medium rounded-lg hover:bg-amber-700 transition-colors shadow-sm" data-testid="btn-tambah"><i class="fas fa-plus mr-1.5"></i> Tambah Pembayaran</button>
        </div>
      </div>
      <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between"><div class="relative w-full sm:w-72"><div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><i class="fas fa-search text-gray-400 text-sm"></i></div><input v-model="searchInput" type="text" placeholder="Cari..." class="w-full pl-10 pr-16 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm focus:ring-2 focus:ring-amber-500 outline-none" @keydown.enter="applySearch" data-testid="input-search"><div class="absolute inset-y-0 right-0 flex items-center gap-1 pr-1.5"><button v-if="searchInput" @click="clearSearch" class="p-1 rounded text-gray-400 hover:text-gray-600 dark:hover:text-white" title="Clear"><i class="fas fa-times text-xs"></i></button><button @click="applySearch" class="px-2 py-1 rounded bg-amber-600 text-white hover:bg-amber-700" title="Cari"><i class="fas fa-search text-xs"></i></button></div></div><div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400"><span>{{ props.pembayarans?.total || 0 }} data</span><button v-if="hasFilter" @click="resetFilters" class="text-xs text-red-500 hover:text-red-700 dark:hover:text-red-400 underline">Reset filter</button></div></div>

      <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 shadow-sm">
        <div class="flex flex-wrap items-end gap-3 sm:gap-4">
          <div class="min-w-[140px] flex-1 sm:flex-none"><label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Provider</label><select v-model="providerFilter" class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-colors"><option value="">Semua</option><option v-for="p in providers" :key="p" :value="p">{{ providerLabel(p) }}</option></select></div>
          <div class="min-w-[140px] flex-1 sm:flex-none"><label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Metode</label><select v-model="paymentMethodFilter" class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-colors"><option value="">Semua</option><option v-for="m in paymentMethods" :key="m" :value="m">{{ paymentMethodLabel(m) }}</option></select></div>
          <div class="min-w-[140px] flex-1 sm:flex-none"><label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Status</label><select v-model="statusFilter" class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-colors"><option value="">Semua</option><option value="paid">Lunas</option><option value="pending">Pending</option><option value="cancelled">Dibatalkan</option><option value="rejected">Ditolak</option><option value="expired">Kedaluarsa</option></select></div>
          <div class="min-w-[130px] flex-1 sm:flex-none"><label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Dari Tgl</label><input v-model="createdStartFilter" type="date" class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-colors" /></div>
          <div class="min-w-[130px] flex-1 sm:flex-none"><label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">S/d</label><input v-model="createdEndFilter" type="date" class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-colors" /></div>
          <div class="min-w-[160px] flex-1 sm:flex-none"><label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Invoice</label><SearchableSelectAjax v-model="invoiceFilter" url="/karyawan/api/search/invoices" placeholder="— Pilih —" display-key="invoice_number" /></div>
          <div class="min-w-[110px] flex-1 sm:flex-none"><label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Terhapus</label><select v-model="terhapusFilter" class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-colors"><option value="tidak">Tidak</option><option value="ya">Ya</option></select></div>
          <button @click="applyFilters" class="px-4 py-2 bg-amber-600 text-white text-sm font-medium rounded-lg hover:bg-amber-700 transition-colors shadow-sm whitespace-nowrap"><i class="fas fa-filter mr-1.5"></i>Filter</button>
        </div>
      </div>
      ...
      <div v-if="hasSelected" class="flex items-center justify-between px-4 py-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl shadow-sm">
        ...
        <div class="flex items-center gap-2">
          <button v-if="can('karyawan-riwayat-pembayaran.persetujuan') && selectedPendingCount > 0" @click="openBulkReview" class="px-3 py-1.5 text-xs font-medium rounded-lg bg-amber-600 text-white hover:bg-amber-700"><i class="fas fa-clipboard-check mr-1"></i> Review ({{ selectedPendingCount }} pending)</button>
          <button v-if="can('karyawan-riwayat-pembayaran.persetujuan') && selectedMidtransPendingCount > 0" @click="bulkVerifyMidtrans()" :disabled="bulkVerifying" class="px-3 py-1.5 text-xs font-medium rounded-lg bg-violet-600 text-white hover:bg-violet-700 disabled:opacity-60 disabled:cursor-not-allowed"><i class="fas fa-sync-alt mr-1"></i> Verifikasi Midtrans ({{ selectedMidtransPendingCount }})</button>
          ...
          <button v-if="can('karyawan-riwayat-pembayaran.delete')" @click="bulkDelete()" class="px-3 py-1.5 text-xs font-medium rounded-lg bg-red-600 text-white hover:bg-red-700"><i class="fas fa-trash-alt mr-1"></i> Hapus</button>
        </div>
      </div>

      <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
          <table data-testid="table-data" class="w-full text-sm min-w-[1400px]">
              ...
              >
                ...
                <td class="px-3 py-3" @click.stop>
                  <div class="flex items-center justify-center gap-1">
                    <button @click="openDetail(item)" title="Detail" class="p-1.5 rounded-lg text-gray-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:text-amber-400 dark:hover:bg-amber-900/30"><i class="fas fa-eye"></i></button>
                    <button v-if="can('karyawan-riwayat-pembayaran.edit') && !item.dihapus && item.status === 'pending' && item.provider === 'internal'" @click="openEdit(item)" title="Edit" class="p-1.5 rounded-lg text-gray-400 hover:text-sky-600 hover:bg-sky-50 dark:hover:text-sky-400 dark:hover:bg-sky-900/30"><i class="fas fa-edit"></i></button>
                    <button v-if="can('karyawan-riwayat-pembayaran.persetujuan') && !item.dihapus && item.status === 'pending' && item.provider === 'internal'" @click="openReview(item)" title="Review" class="p-1.5 rounded-lg text-gray-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:text-amber-400 dark:hover:bg-amber-900/30"><i class="fas fa-clipboard-check"></i></button>
                    ...
                    <button v-if="can('karyawan-riwayat-pembayaran.delete') && !item.dihapus && item.provider === 'internal'" @click="openDelete(item)" title="Hapus" class="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:text-red-400 dark:hover:bg-red-900/30"><i class="fas fa-trash-alt"></i></button>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showCreateModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><form data-testid="modal-create" @submit.prevent="submitCreate" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col"><div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-plus text-emerald-500 mr-2"></i>Tambah Pembayaran</h3><button type="button" @click="showCreateModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><div class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll">
        <FormErrorSummary :errors="createForm.errors" testId="form-error-summary-create-pembayaran" />
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Kode Pembayaran <span class="text-red-500">*</span></label><input data-testid="input-code" v-model="createForm.code" type="text" placeholder="Contoh: BYR-20250601-XXXX" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 outline-none', createForm.errors.code ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-amber-500']" /><p v-if="createForm.errors.code" class="text-red-500 text-xs mt-1">{{ createForm.errors.code }}</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Invoice <span class="text-red-500">*</span></label><span data-testid="btn-select-invoice"><SearchableSelectAjax v-model="createForm.cust_internet_invc_id" url="/karyawan/api/search/invoices" placeholder="— Pilih Invoice —" :error="!!createForm.errors.cust_internet_invc_id" /></span><p v-if="createForm.errors.cust_internet_invc_id" class="text-red-500 text-xs mt-1">{{ createForm.errors.cust_internet_invc_id }}</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Jumlah (Rp) <span class="text-red-500">*</span></label><input data-testid="input-amount" v-model="createForm.amount_paid" type="number" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 outline-none', createForm.errors.amount_paid ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-amber-500']" /><p v-if="createForm.errors.amount_paid" class="text-red-500 text-xs mt-1">{{ createForm.errors.amount_paid }}</p></div>
        <div class="grid grid-cols-2 gap-4"><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Provider <span class="text-red-500">*</span></label><select data-testid="select-provider" v-model="createForm.provider" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 outline-none', createForm.errors.provider ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-amber-500']"><option value="">— Pilih —</option><option v-for="p in providers" :key="p" :value="p">{{ providerLabel(p) }}</option></select><p v-if="createForm.errors.provider" class="text-red-500 text-xs mt-1">{{ createForm.errors.provider }}</p></div><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Metode <span class="text-red-500">*</span></label><select data-testid="select-metode" v-model="createForm.payment_method" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 outline-none', createForm.errors.payment_method ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-amber-500']"><option value="">— Pilih —</option><option v-for="m in paymentMethods" :key="m" :value="m">{{ paymentMethodLabel(m) }}</option></select><p v-if="createForm.errors.payment_method" class="text-red-500 text-xs mt-1">{{ createForm.errors.payment_method }}</p></div></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Bukti Bayar (jpg, png, webp, pdf) <span class="text-xs text-gray-400">(opsional, maks 2MB)</span></label><input @change="e => createFormFile = e.target.files[0]" type="file" accept=".jpg,.jpeg,.png,.webp,.pdf" class="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:bg-amber-50 file:text-amber-700 dark:file:bg-amber-900/30 dark:file:text-amber-400 hover:file:bg-amber-100 dark:hover:file:bg-amber-900/50" /><p v-if="createForm.errors.proof_file" class="text-red-500 text-xs mt-1">{{ createForm.errors.proof_file }}</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Keterangan</label><textarea v-model="createForm.status_description" rows="2" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 outline-none resize-none', createForm.errors.status_description ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-amber-500']"></textarea><p v-if="createForm.errors.status_description" class="text-red-500 text-xs mt-1">{{ createForm.errors.status_description }}</p></div>
    ...
    <!-- Edit Modal -->
    <Teleport to="body"><Transition name="modal"><div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showEditModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><form @submit.prevent="submitEdit" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col"><div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-edit text-amber-500 mr-2"></i>Edit Pembayaran</h3><button type="button" @click="showEditModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><div class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll">
        <FormErrorSummary :errors="editForm.errors" testId="form-error-summary-edit-pembayaran" />
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Kode Pembayaran <span class="text-red-500">*</span></label><input v-model="editForm.code" type="text" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-900 text-gray-500 dark:text-gray-400 text-sm focus:ring-2 focus:ring-amber-500 outline-none" readonly /><p class="text-xs text-gray-400 mt-1">Kode tidak dapat diubah.</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Invoice <span class="text-red-500">*</span></label><SearchableSelectAjax v-model="editForm.cust_internet_invc_id" url="/karyawan/api/search/invoices" placeholder="— Pilih Invoice —" :selected-label="selectedItem?.invoice_number" :error="!!editForm.errors.cust_internet_invc_id" /><p v-if="editForm.errors.cust_internet_invc_id" class="text-red-500 text-xs mt-1">{{ editForm.errors.cust_internet_invc_id }}</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Jumlah (Rp) <span class="text-red-500">*</span></label><input v-model="editForm.amount_paid" type="number" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 outline-none', editForm.errors.amount_paid ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-amber-500']" /><p v-if="editForm.errors.amount_paid" class="text-red-500 text-xs mt-1">{{ editForm.errors.amount_paid }}</p></div>
        <div class="grid grid-cols-2 gap-4"><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Provider <span class="text-red-500">*</span></label><select v-model="editForm.provider" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 outline-none', editForm.errors.provider ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-amber-500']"><option value="">— Pilih —</option><option v-for="p in providers" :key="p" :value="p">{{ providerLabel(p) }}</option></select><p v-if="editForm.errors.provider" class="text-red-500 text-xs mt-1">{{ editForm.errors.provider }}</p></div><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Metode <span class="text-red-500">*</span></label><select v-model="editForm.payment_method" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 outline-none', editForm.errors.payment_method ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-amber-500']"><option value="">— Pilih —</option><option v-for="m in paymentMethods" :key="m" :value="m">{{ paymentMethodLabel(m) }}</option></select><p v-if="editForm.errors.payment_method" class="text-red-500 text-xs mt-1">{{ editForm.errors.payment_method }}</p></div></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Bukti Bayar <span class="text-xs text-gray-400">(kosongkan jika tidak diubah, maks 2MB)</span></label><input @change="e => editFormFile = e.target.files[0]" type="file" accept=".jpg,.jpeg,.png,.webp,.pdf" class="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:bg-amber-50 file:text-amber-700 dark:file:bg-amber-900/30 dark:file:text-amber-400 hover:file:bg-amber-100 dark:hover:file:bg-amber-900/50" /><p v-if="editForm.errors.proof_file" class="text-red-500 text-xs mt-1">{{ editForm.errors.proof_file }}</p></div>
        <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Keterangan</label><textarea v-model="editForm.status_description" rows="2" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 outline-none resize-none', editForm.errors.status_description ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-amber-500']"></textarea><p v-if="editForm.errors.status_description" class="text-red-500 text-xs mt-1">{{ editForm.errors.status_description }}</p></div>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showReviewModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showReviewModal = false; reviewForm.reset(); bulkReviewForm.reset();"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div>
      ...
      <form v-if="selectedItem && !hasSelected" @submit.prevent="submitReview" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col">
        <div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-clipboard-check text-amber-500 mr-2"></i>Review: {{ selectedItem?.code }}</h3><button type="button" @click="showReviewModal = false; reviewForm.reset();" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div>
        <div class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll">
          <FormErrorSummary :errors="reviewForm.errors" testId="form-error-summary-review" />
          ...
          <div class="bg-gray-50 dark:bg-gray-900 rounded-lg p-3"><div class="text-xs text-gray-500 dark:text-gray-400 mb-1">Nominal</div><div class="font-mono font-semibold text-amber-600">Rp {{ Number(selectedItem?.amount_paid || 0).toLocaleString('id') }}</div></div>
          <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Status Review <span class="text-red-500">*</span></label><select v-model="reviewForm.review_status" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 outline-none', reviewForm.errors.review_status ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-amber-500']"><option value="">— Pilih —</option><option value="approved">Disetujui</option><option value="rejected">Ditolak</option></select><p v-if="reviewForm.errors.review_status" class="text-red-500 text-xs mt-1">{{ reviewForm.errors.review_status }}</p></div>
          <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Alasan <span v-if="reviewForm.review_status === 'rejected'" class="text-red-500">*</span><span v-else class="text-xs text-gray-400">(wajib jika ditolak)</span></label><textarea v-model="reviewForm.review_reason" rows="3" placeholder="Jelaskan alasan persetujuan atau penolakan..." :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 outline-none resize-none', reviewForm.errors.review_reason ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-amber-500']"></textarea><p v-if="reviewForm.errors.review_reason" class="text-red-500 text-xs mt-1">{{ reviewForm.errors.review_reason }}</p></div>
          <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Lampiran Review <span class="text-xs text-gray-400">(opsional)</span></label><input @change="reviewForm.review_attachment = $event.target.files[0]" type="file" accept=".jpg,.jpeg,.png,.pdf" class="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:bg-amber-50 file:text-amber-700 dark:file:bg-amber-900/30 dark:file:text-amber-400 hover:file:bg-amber-100 dark:hover:file:bg-amber-900/50" /><p v-if="reviewForm.errors.review_attachment" class="text-red-500 text-xs mt-1">{{ reviewForm.errors.review_attachment }}</p></div>
        </div>
        <div class="shrink-0 flex justify-end gap-2 px-6 py-4 border-t border-gray-200 dark:border-gray-700"><button type="button" @click="showReviewModal = false; reviewForm.reset();" class="px-4 py-2.5 bg-gray-200 dark:bg-gray-700 text-sm font-medium rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">Batal</button><button type="submit" :disabled="reviewForm.processing" class="px-6 py-2.5 bg-amber-600 text-white text-sm font-medium rounded-lg hover:bg-amber-700 transition-colors shadow-sm disabled:opacity-50"><i class="fas fa-check mr-1.5"></i>Simpan Review</button></div>
      </form>
      <!-- Bulk Review -->
      <form v-else @submit.prevent="submitBulkReview" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col">
        <div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-clipboard-check text-amber-500 mr-2"></i>Bulk Review ({{ selectedIds.length }} item)</h3><button type="button" @click="showReviewModal = false; bulkReviewForm.reset();" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div>
        <div class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll">
          <FormErrorSummary :errors="bulkReviewForm.errors" testId="form-error-summary-bulk-review" />
          ...
          <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Status Review <span class="text-red-500">*</span></label><select v-model="bulkReviewForm.review_status" :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 outline-none', bulkReviewForm.errors.review_status ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-amber-500']"><option value="">— Pilih —</option><option value="approved">Disetujui</option><option value="rejected">Ditolak</option></select><p v-if="bulkReviewForm.errors.review_status" class="text-red-500 text-xs mt-1">{{ bulkReviewForm.errors.review_status }}</p></div>
          <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Alasan <span v-if="bulkReviewForm.review_status === 'rejected'" class="text-red-500">*</span><span v-else class="text-xs text-gray-400">(wajib jika ditolak)</span></label><textarea v-model="bulkReviewForm.review_reason" rows="3" placeholder="Jelaskan alasan persetujuan atau penolakan..." :class="['w-full px-3 py-2.5 rounded-lg border bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 outline-none resize-none', bulkReviewForm.errors.review_reason ? 'border-red-400 focus:ring-red-500' : 'border-gray-300 dark:border-gray-600 focus:ring-amber-500']"></textarea><p v-if="bulkReviewForm.errors.review_reason" class="text-red-500 text-xs mt-1">{{ bulkReviewForm.errors.review_reason }}</p></div>
          <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Lampiran Review <span class="text-xs text-gray-400">(opsional)</span></label><input @change="bulkReviewForm.review_attachment = $event.target.files[0]" type="file" accept=".jpg,.jpeg,.png,.pdf" class="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:bg-amber-50 file:text-amber-700 dark:file:bg-amber-900/30 dark:file:text-amber-400 hover:file:bg-amber-100 dark:hover:file:bg-amber-900/50" /><p v-if="bulkReviewForm.errors.review_attachment" class="text-red-500 text-xs mt-1">{{ bulkReviewForm.errors.review_attachment }}</p></div>
        </div>
        <div class="shrink-0 flex justify-end gap-2 px-6 py-4 border-t border-gray-200 dark:border-gray-700"><button type="button" @click="showReviewModal = false; bulkReviewForm.reset();" class="px-4 py-2.5 bg-gray-200 dark:bg-gray-700 text-sm font-medium rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">Batal</button><button type="submit" :disabled="bulkReviewForm.processing" class="px-6 py-2.5 bg-amber-600 text-white text-sm font-medium rounded-lg hover:bg-amber-700 transition-colors shadow-sm disabled:opacity-50"><i class="fas fa-check mr-1.5"></i>Simpan Bulk Review</button></div>
      </form>
    </div></Transition></Teleport>

    <!-- Import Modal -->
    <Teleport to="body"><Transition name="modal"><div v-if="showImportModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showImportModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><form @submit.prevent="submitImport" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col"><div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-file-import text-emerald-500 mr-2"></i>Import Pembayaran</h3><button type="button" @click="showImportModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white"><i class="fas fa-times"></i></button></div><div class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll"><FormErrorSummary :errors="importForm.errors" testId="form-error-summary-import-pembayaran" /><div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg px-4 py-3 text-sm text-amber-700 dark:text-amber-300"><i class="fas fa-info-circle mr-1.5"></i> Pastikan file Excel sesuai template. Kolom: No. Invoice, Jumlah Bayar, Provider, Metode Pembayaran.</div><div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">File Excel <span class="text-red-500">*</span></label><input @change="onImportFileChange" type="file" accept=".xlsx,.xls" class="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:bg-amber-50 file:text-amber-700 dark:file:bg-amber-900/30 dark:file:text-amber-400 hover:file:bg-amber-100 dark:hover:file:bg-amber-900/50" /><p v-if="importForm.errors.file" class="text-red-500 text-xs mt-1">{{ importForm.errors.file }}</p></div></div><div class="shrink-0 flex justify-end gap-2 px-6 py-4 border-t border-gray-200 dark:border-gray-700"><button type="button" @click="showImportModal = false" class="px-4 py-2.5 bg-gray-200 dark:bg-gray-700 text-sm font-medium rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600">Batal</button><button type="submit" :disabled="importing" class="px-6 py-2.5 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 disabled:opacity-50"><i class="fas fa-upload mr-1.5"></i>{{ importing ? 'Importing...' : 'Import' }}</button></div></form></div></Transition></Teleport>
  </div>
</template>
