...
async function openDetail(item) {
    ...
    try {
        const res = await fetch(`/karyawan/api/tagihan/${item.id}/payments`, {
...
function confirmDelete() { router.delete('/karyawan/tagihan/' + selectedItem.value.id, { onSuccess: () => { showDeleteModal.value = false; fetchData(); toast.success('Tagihan berhasil dihapus.'); } }); }
function confirmRestore(id) { router.patch('/karyawan/tagihan/' + id + '/restore', { onSuccess: () => { fetchData(); toast.success('Tagihan berhasil dipulihkan.'); } }); }
function bulkDelete() { router.post('/karyawan/tagihan/bulk-delete', { ids: selectedIds.value }, { onSuccess: () => { selectedIds.value = []; selectAll.value = false; fetchData(); toast.success('Tagihan berhasil dihapus.'); } }); }
function bulkRestore() { router.post('/karyawan/tagihan/bulk-restore', { ids: selectedIds.value }, { onSuccess: () => { selectedIds.value = []; selectAll.value = false; fetchData(); toast.success('Tagihan berhasil dipulihkan.'); } }); }
...
function exportAll() {
    ...
    window.location.href = '/karyawan/tagihan/export' + (params ? '?' + params : '');
}
function exportSelected() {
    if (selectedIds.value.length > 0) {
        window.location.href = '/karyawan/tagihan/export?ids=' + selectedIds.value.join(',');
    } else {
        const params = buildFilterParams();
        window.location.href = '/karyawan/tagihan/export' + (params ? '?' + params : '');
    }
  ...
  <div>
    ...
    <div class="space-y-6">
      <nav class="flex items-center gap-1.5 text-sm">
        ...
        <span class="text-gray-900 dark:text-white font-medium">Tagihan</span>
      </nav>

      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Tagihan</h2>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Kelola tagihan bulanan pelanggan.</p>
        </div>
        <div class="flex flex-wrap items-center gap-2">
          ...
          <div v-if="can('karyawan-tagihan.export')" class="relative group">
            <button class="inline-flex items-center px-3 py-2 bg-amber-600 text-white text-sm font-medium rounded-lg hover:bg-amber-700 transition-colors shadow-sm">
      ...
      <div class="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
          <span>{{ props.tagihans?.total || 0 }} data</span>
      ...
      <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 shadow-sm">
        <div class="flex flex-wrap items-end gap-3">
          ...
          <div class="min-w-[220px]">
            ...
            <SearchableSelectAjax v-model="paketFilter" url="/karyawan/api/search/packages" placeholder="Semua Paket" :page-size="25" label-key="label" value-key="value" />
          </div>
          <div class="min-w-[150px]">
            ...
            <input v-model="dueDateStart" type="date" class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-colors dark:[color-scheme:dark]" />
          </div>
          <div class="min-w-[150px]">
            ...
            <input v-model="dueDateEnd" type="date" class="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-colors dark:[color-scheme:dark]" />
          </div>
          <div class="min-w-[160px]">
      ...
      <div v-if="selectedIds.length > 0" class="flex items-center justify-between px-4 py-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl shadow-sm">
        ...
        <div class="flex items-center gap-2">
          <template v-if="terhapusFilter === 'ya'">
            <button v-if="can('karyawan-tagihan.restore')" @click="bulkRestore()" class="px-3 py-1.5 text-xs font-medium rounded-lg bg-emerald-600 text-white hover:bg-emerald-700"><i class="fas fa-undo-alt mr-1"></i> Pulihkan</button>
            <button v-if="can('karyawan-tagihan.delete')" @click="bulkDelete()" class="px-3 py-1.5 text-xs font-medium rounded-lg bg-red-600 text-white hover:bg-red-700"><i class="fas fa-trash-alt mr-1"></i> Hapus</button>
          </template>
          <template v-else>
            <button v-if="can('karyawan-tagihan.edit')" @click="bulkSetStatus('paid')" class="px-3 py-1.5 text-xs font-medium rounded-lg bg-emerald-600 text-white hover:bg-emerald-700"><i class="fas fa-check mr-1"></i> Set Lunas</button>
            <button v-if="can('karyawan-tagihan.edit')" @click="bulkSetStatus('unpaid')" class="px-3 py-1.5 text-xs font-medium rounded-lg bg-amber-600 text-white hover:bg-amber-700"><i class="fas fa-times mr-1"></i> Set Belum Bayar</button>
            <button v-if="can('karyawan-tagihan.delete')" @click="bulkDelete()" class="px-3 py-1.5 text-xs font-medium rounded-lg bg-red-600 text-white hover:bg-red-700"><i class="fas fa-trash-alt mr-1"></i> Hapus</button>
          </template>
        </div>
      </div>
          ...
          <table class="w-full text-sm min-w-[1200px]">
            <thead class="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
              <tr>
                ...
                <th class="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-400">Nama Pelanggan</th>
                <th class="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-400">No. Telp</th>
                <th class="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-400">Email</th>
                <th @click="sort('usage_start_date')" class="px-4 py-3 text-center font-semibold text-gray-600 dark:text-gray-400 cursor-pointer select-none"><span class="inline-flex items-center gap-1">Awal Usage <i :class="['fas', sortIcon('usage_start_date'), 'text-[10px]', sortField === 'usage_start_date' ? 'text-amber-500' : 'text-gray-400']"></i></span></th>
                <th @click="sort('usage_end_date')" class="px-4 py-3 text-center font-semibold text-gray-600 dark:text-gray-400 cursor-pointer select-none"><span class="inline-flex items-center gap-1">Akhir Usage <i :class="['fas', sortIcon('usage_end_date'), 'text-[10px]', sortField === 'usage_end_date' ? 'text-amber-500' : 'text-gray-400']"></i></span></th>
                <th @click="sort('total_amount')" class="px-4 py-3 text-right font-semibold text-gray-600 dark:text-gray-400 cursor-pointer select-none"><span class="inline-flex items-center gap-1">Total <i :class="['fas', sortIcon('total_amount'), 'text-[10px]', sortField === 'total_amount' ? 'text-amber-500' : 'text-gray-400']"></i></span></th>
                <th class="px-4 py-3 text-right font-semibold text-gray-600 dark:text-gray-400">Diskon</th>
                <th class="px-4 py-3 text-right font-semibold text-gray-600 dark:text-gray-400">Pajak</th>
                <th class="px-4 py-3 text-right font-semibold text-gray-600 dark:text-gray-400">Grand Total</th>
                <th @click="sort('due_date')" class="px-4 py-3 text-left font-semibold text-gray-600 dark:text-gray-400 cursor-pointer select-none"><span class="inline-flex items-center gap-1">Jatuh Tempo <i :class="['fas', sortIcon('due_date'), 'text-[10px]', sortField === 'due_date' ? 'text-amber-500' : 'text-gray-400']"></i></span></th>
                <th @click="sort('payment_status')" class="px-4 py-3 text-center font-semibold text-gray-600 dark:text-gray-400 cursor-pointer select-none"><span class="inline-flex items-center gap-1">Status <i :class="['fas', sortIcon('payment_status'), 'text-[10px]', sortField === 'payment_status' ? 'text-amber-500' : 'text-gray-400']"></i></span></th>
                <th class="px-4 py-3 text-right font-semibold text-gray-600 dark:text-gray-400">Pembayaran Lunas</th>
                <th class="px-4 py-3 text-right font-semibold text-gray-600 dark:text-gray-400">Sisa Kurang Pembayaran</th>
                <th class="px-4 py-3 text-center font-semibold text-gray-600 dark:text-gray-400 w-24">Aksi</th>
              </tr>
            </thead>
            <tbody>
              <tr v-if="items.length === 0">
                <td colspan="15" class="px-4 py-16 text-center text-gray-400 dark:text-gray-500">
                  <i class="fas fa-inbox text-4xl mb-3 block"></i>
                  <span class="text-sm">Tidak ada data tagihan</span>
                </td>
              </tr>
              <tr v-for="item in items" :key="item.id" class="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors cursor-pointer" :class="{'opacity-60': item.dihapus}" @click="(e) => { if (!e.target.closest('button') && !e.target.closest('input')) toggleSelect(item.id); }">
                <td class="px-4 py-3" @click.stop><input v-model="selectedIds" :value="item.id" type="checkbox" class="rounded border-gray-300 text-amber-600 cursor-pointer" /></td>
                ...
                <td class="px-4 py-3 text-center">
                  <span :class="['inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium', statusBadgeClass(item.payment_status_label)]">{{ statusLabel(item.payment_status_label) }}</span>
                </td>
                ...
                <td class="px-4 py-3" @click.stop>
                  <div class="flex items-center justify-center gap-1">
                    <button v-if="can('karyawan-tagihan.detail') && !item.dihapus" @click="openDetail(item)" title="Detail" class="p-1.5 rounded-lg text-gray-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:text-amber-400 dark:hover:bg-amber-900/30"><i class="fas fa-eye"></i></button>
                    <button v-if="can('karyawan-tagihan.edit') && !item.dihapus" @click="openEdit(item)" title="Edit" class="p-1.5 rounded-lg text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:text-indigo-400 dark:hover:bg-indigo-900/30"><i class="fas fa-edit"></i></button>
                    <button v-if="can('karyawan-tagihan.delete') && !item.dihapus" @click="openDelete(item)" title="Hapus" class="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:text-red-400 dark:hover:bg-red-900/30"><i class="fas fa-trash-alt"></i></button>
                    <button v-if="can('karyawan-tagihan.restore') && item.dihapus" @click="confirmRestore(item.id)" title="Pulihkan" class="p-1.5 rounded-lg text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:text-emerald-400 dark:hover:bg-emerald-900/30"><i class="fas fa-undo"></i></button>
                  </div>
                </td>
              </tr>
            </tbody>
        ...
        <div v-if="showDetailModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showDetailModal = false">
          ...
          <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
            <div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
              <h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-eye mr-2 text-amber-500"></i>Detail Tagihan</h3>
            ...
            <div class="overflow-y-auto flex-1 px-6 py-5 space-y-5 dark:text-white modal-scroll" v-if="selectedItem">
              ...
              <div>
                <div class="flex items-center justify-between mb-2">
                  <h4 class="text-sm font-semibold text-gray-900 dark:text-white">Riwayat Pembayaran</h4>
                  <span v-if="paymentHistory.payments.length > 0" :class="['inline-flex px-2 py-0.5 rounded-full text-xs font-medium', statusBadgeClass(paymentHistory.payment_status_label)]">{{ statusLabel(paymentHistory.payment_status_label) }}</span>
                </div>
                  ...
                  <table class="w-full text-xs">
                    <thead>
                      <tr class="bg-gray-50 dark:bg-gray-900">
                        ...
                        <th class="px-2 py-1.5 text-center font-semibold text-gray-600 dark:text-gray-400">Tgl Bayar</th>
                        <th class="px-2 py-1.5 text-right font-semibold text-gray-600 dark:text-gray-400">Nominal</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr v-for="(p, idx) in paymentHistory.payments" :key="p.id" :class="['border-b border-gray-100 dark:border-gray-700', idx % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50/50 dark:bg-gray-900/30']">
                        <td class="px-2 py-1.5 font-mono text-gray-900 dark:text-white">{{ p.code }}</td>
                        <td class="px-2 py-1.5 text-center text-gray-900 dark:text-white">{{ p.payment_date }}</td>
                        <td class="px-2 py-1.5 text-right font-mono text-gray-900 dark:text-white">Rp {{ Number(p.amount_paid || 0).toLocaleString('id') }}</td>
                      </tr>
                    </tbody>
                    <tfoot>
                      <tr class="border-t-2 border-gray-300 dark:border-gray-600">
                        <td colspan="2" class="px-2 py-1.5 text-right text-xs font-semibold text-gray-700 dark:text-gray-300">Total Pembayaran:</td>
                        <td class="px-2 py-1.5 text-right font-mono font-semibold text-emerald-700 dark:text-emerald-400">Rp {{ Number(paymentHistory.total_paid || 0).toLocaleString('id') }}</td>
                      </tr>
                      <tr>
                        <td colspan="2" class="px-2 py-1.5 text-right text-xs font-semibold text-gray-700 dark:text-gray-300">Sisa yang belum dibayar:</td>
                        <td class="px-2 py-1.5 text-right font-mono font-semibold" :class="paymentHistory.remaining > 0 ? 'text-red-700 dark:text-red-400' : 'text-emerald-700 dark:text-emerald-400'">Rp {{ Number(paymentHistory.remaining || 0).toLocaleString('id') }}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
              </div>
              <div v-if="can('karyawan-tagihan.export')" class="flex items-center justify-end gap-2 pt-4 border-t border-gray-200 dark:border-gray-700">
                <a :href="`/karyawan/tagihan/${selectedItem.id}/export-pdf`" target="_blank" class="inline-flex items-center px-3 py-1.5 bg-red-600 text-white text-xs font-medium rounded-lg hover:bg-red-700 transition-colors">
                  <i class="fas fa-file-pdf mr-1.5"></i>Export PDF
                </a>
                <a :href="`/karyawan/tagihan/${selectedItem.id}/export-word`" target="_blank" class="inline-flex items-center px-3 py-1.5 bg-blue-600 text-white text-xs font-medium rounded-lg hover:bg-blue-700 transition-colors">
                  <i class="fas fa-file-word mr-1.5"></i>Export Word
                </a>
              </div>
            </div>
          </div>
      ...
      <Transition name="modal">
        <div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showCreateModal = false">
          ...
          <form @submit.prevent="submitCreate" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
            <div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
              <h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-plus text-emerald-500 mr-2"></i>Tambah Tagihan</h3>
            ...
            <div class="shrink-0 flex justify-end gap-2 px-6 py-4 border-t border-gray-200 dark:border-gray-700">
              ...
              <button type="submit" :disabled="createForm.processing" class="px-6 py-2.5 bg-amber-600 text-white text-sm font-medium rounded-lg hover:bg-amber-700 transition-colors shadow-sm disabled:opacity-50"><i class="fas fa-save mr-1.5"></i>Simpan</button>
            </div>
          </form>
        </div>
      </Transition>
      ...
      <Transition name="modal">
        <div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showEditModal = false">
          ...
          <form @submit.prevent="submitEdit" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
            <div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
              <h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-edit text-amber-500 mr-2"></i>Edit Tagihan</h3>
            ...
            <div class="shrink-0 flex justify-end gap-2 px-6 py-4 border-t border-gray-200 dark:border-gray-700">
              ...
              <button type="submit" :disabled="editForm.processing" class="px-6 py-2.5 bg-amber-600 text-white text-sm font-medium rounded-lg hover:bg-amber-700 transition-colors shadow-sm disabled:opacity-50"><i class="fas fa-check mr-1.5"></i>Update</button>
            </div>
          </form>
        </div>
      </Transition>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showGenerateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showGenerateModal = false">
          <div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div>
          <form @submit.prevent="submitGenerate" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
              <h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-magic text-purple-500 mr-2"></i>Generate Tagihan Massal</h3>
            ...
            <div class="flex justify-end gap-2 px-6 py-4 border-t border-gray-200 dark:border-gray-700">
              <button type="button" @click="showGenerateModal = false" class="px-4 py-2.5 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">Batal</button>
              <button type="submit" :disabled="generateForm.processing" class="px-6 py-2.5 bg-purple-600 text-white text-sm font-medium rounded-lg hover:bg-purple-700 transition-colors shadow-sm disabled:opacity-50"><i class="fas fa-magic mr-1.5"></i>Generate</button>
            </div>
          </form>
        </div>
      </Transition>
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="showImportModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showImportModal = false">
          <div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div>
          <form @submit.prevent="submitImport" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
              <h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-file-import text-emerald-500 mr-2"></i>Import Tagihan</h3>
            ...
            <div class="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <input @change="importForm.file = $event.target.files[0]" type="file" accept=".xlsx,.csv" class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-amber-50 file:text-amber-700 hover:file:bg-amber-100" />
            ...
            <div class="flex justify-end gap-2 px-6 py-4 border-t border-gray-200 dark:border-gray-700">
              ...
              <button type="submit" :disabled="importing || !importForm.file" class="px-6 py-2.5 bg-emerald-600 text-white text-sm font-medium rounded-lg hover:bg-emerald-700 transition-colors shadow-sm disabled:opacity-50">
                <i :class="['fas', importing ? 'fa-spinner fa-spin' : 'fa-upload', 'mr-1.5']"></i>{{ importing ? 'Mengimport...' : 'Import' }}
              </button>
            </div>
          </form>
        </div>
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showDeleteModal = false">
          ...
          <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-sm">
            <div class="px-6 py-5 text-center">
              ...
              <p class="text-sm text-gray-500 dark:text-gray-400">Anda akan menghapus tagihan <strong>{{ selectedItem?.invoice_number }}</strong> dari <strong>{{ selectedItem?.customer_name }}</strong>.</p>
            </div>
