...
const editForm = useForm({ assigned_to_employee_id: '', catatan: '', status_pengerjaan: '', file_bukti_issue_diselesaikan: null });
const resolveForm = useForm({ file_bukti_issue_diselesaikan: null });
const createFormFile = ref(null);
const editFormFile = ref(null);
const resolveFormFile = ref(null);
...
function openResolve(item) { resolveForm.reset(); resolveForm.clearErrors(); resolveFormFile.value = null; selectedItem.value = item; showResolveModal.value = true; }
async function submitResolve() {
  const fd = new FormData();
  fd.append('file_bukti_issue_diselesaikan', resolveFormFile.value || '');
  try {
    const csrf = document.querySelector('meta[name="csrf-token"]')?.content || '';
    ...
    const data = await resp.json().catch(() => ({}));
    if (resp.ok) { showResolveModal.value = false; toast.success('Tiket ditandai selesai. Menunggu verifikasi admin.'); fetchData(); }
    else { toast.error(data.message || `Gagal (HTTP ${resp.status})`); }
  } catch (e) { toast.error('Error: ' + e.message); }
}
...
<template>
  ...
  <div class="space-y-6">
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="item in items" :key="item.id" class="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-900/30">
              ...
              <td class="px-3 py-3 text-xs text-gray-600 dark:text-gray-400">{{ item.assigned_to_name || '— Belum di-assign' }}</td>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showCreateModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><form data-testid="modal-create" @submit.prevent="submitCreate" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col"><div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-plus text-emerald-500 mr-2"></i>Buat Tiket Gangguan</h3><button type="button" @click="showCreateModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><div class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll">
      <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Kode Langganan <span class="text-red-500">*</span></label><span data-testid="btn-select-cust-internet"><select data-testid="select-cust-internet" v-model="createForm.cust_internet_id" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-amber-500 outline-none"><option value="">— Pilih Kode Langganan —</option><option v-for="ci in custInternets" :key="ci.id" :value="ci.id">{{ ci.display }}</option></select></span></div>
      ...
      <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Foto Bukti (opsional)</label><input data-testid="input-file-bukti" @change="e => createFormFile = e.target.files[0]" type="file" accept=".jpg,.jpeg,.png,.webp,.pdf" class="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:bg-amber-50 file:text-amber-700 dark:file:bg-amber-900/30 dark:file:text-amber-400 hover:file:bg-amber-100 dark:hover:file:bg-amber-900/50" /></div>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showEditModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><form data-testid="modal-edit" @submit.prevent="submitEdit" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col"><div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-edit text-amber-500 mr-2"></i>Edit Tiket</h3><button type="button" @click="showEditModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><div class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll">
      <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Penanggung Jawab</label><select data-testid="select-assigned" v-model="editForm.assigned_to_employee_id" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-amber-500 outline-none"><option value="">— Belum di-assign —</option><option v-for="e in employees" :key="e.id" :value="e.id">{{ e.name }}</option></select></div>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showResolveModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showResolveModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><form data-testid="modal-resolve" @submit.prevent="submitResolve" class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col"><div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white"><i class="fas fa-check-circle text-emerald-500 mr-2"></i>Tandai Selesai</h3><button type="button" @click="showResolveModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-times"></i></button></div><div class="overflow-y-auto flex-1 px-6 py-5 space-y-4 modal-scroll">
      <p class="text-sm text-gray-600 dark:text-gray-400">Tandai tiket <strong class="text-gray-900 dark:text-white">{{ selectedItem?.code }}</strong> sebagai selesai. Upload foto bukti kalau ada.</p>
      <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Bukti Issue Selesai</label><input data-testid="input-file-bukti-selesai" @change="e => resolveFormFile = e.target.files[0]" type="file" accept=".jpg,.jpeg,.png,.webp,.pdf" class="w-full text-sm text-gray-500 dark:text-gray-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:bg-emerald-50 file:text-emerald-700 dark:file:bg-emerald-900/30 dark:file:text-emerald-400 hover:file:bg-emerald-100 dark:hover:file:bg-emerald-900/50" /></div>
    ...
    <Teleport to="body"><Transition name="modal"><div v-if="showDetailModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showDetailModal = false"><div class="fixed inset-0 bg-black/50 backdrop-blur-sm"></div><div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col"><div class="shrink-0 flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="text-lg font-semibold text-gray-900 dark:text-white">Detail Tiket</h3><button @click="showDetailModal = false" class="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700"><i class="fas fa-times"></i></button></div><div class="overflow-y-auto flex-1 px-6 py-5 modal-scroll" v-if="selectedItem">
      <div class="grid grid-cols-2 gap-4">
        ...
        <div v-if="selectedItem.file_bukti_issue_url"><label class="text-xs text-gray-500 dark:text-gray-400">Bukti Issue</label><a :href="selectedItem.file_bukti_issue_url" target="_blank" class="inline-flex items-center mt-1 px-3 py-1.5 bg-amber-600 text-white text-xs rounded-lg hover:bg-amber-700"><i class="fas fa-image mr-1"></i>Lihat</a></div>
        <div v-if="selectedItem.file_bukti_issue_diselesaikan_url"><label class="text-xs text-gray-500 dark:text-gray-400">Bukti Selesai</label><a :href="selectedItem.file_bukti_issue_diselesaikan_url" target="_blank" class="inline-flex items-center mt-1 px-3 py-1.5 bg-amber-600 text-white text-xs rounded-lg hover:bg-amber-700"><i class="fas fa-image mr-1"></i>Lihat</a></div>
      </div>
