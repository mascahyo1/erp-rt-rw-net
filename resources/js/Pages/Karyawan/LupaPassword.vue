...
const submit = () => {
    if (isResetMode.value) {
        ...
        form.transform((data) => ({ ...data, company_id: selectedCompany.value.id })).post('/lupa-password-karyawan', {
            onSuccess: () => {
...
<template>
    <Head title="Lupa Password - Karyawan" />
    <ToastContainer />
    <section class="min-h-[calc(100vh-200px)] flex overflow-x-hidden">
        <div class="flex-1 grid grid-cols-1 lg:grid-cols-2 min-w-0">
            <div class="relative overflow-hidden bg-gradient-to-br from-amber-600 via-orange-700 to-amber-900 flex items-center justify-center px-4 sm:px-6 py-16 lg:py-0 min-w-0">
                ...
                <div class="relative text-center text-white max-w-sm">
                    ...
                    <p class="text-amber-100 leading-relaxed text-sm md:text-base">
                        Pilih perusahaan Anda dan masukkan email karyawan untuk menerima link reset.
                    </p>
                </div>
            </div>
                    ...
                    <div class="rounded-2xl bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-8 shadow-lg">
                        <div class="text-center mb-6">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                {{ isResetMode ? 'Masukkan password baru untuk akun karyawan Anda.' : 'Masukkan email dan perusahaan Anda untuk menerima link reset.' }}
                            </p>
                        </div>
...
<p v-if="form.errors.email" class="text-red-500 text-xs mt-1">{{ form.errors.email }}</p>
                            ...
                            <template v-else>
                                <div class="rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 p-3 text-sm">
                                    <p class="text-amber-700 dark:text-amber-300"><i class="fas fa-info-circle mr-1.5"></i>Email: <strong>{{ props.email }}</strong></p>
                                </div>
...
<p v-if="form.errors.password" class="text-red-500 text-xs mt-1">{{ form.errors.password }}</p>
                        ...
                        <div class="mt-6 text-center text-sm">
                            <Link href="/login-karyawan" class="text-amber-600 dark:text-amber-400 hover:underline">
