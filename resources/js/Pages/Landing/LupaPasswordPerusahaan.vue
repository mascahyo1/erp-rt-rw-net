...
const submit = () => {
    if (isResetMode.value) {
        form.post('/lupa-password-perusahaan/reset', {
        ...
        form.transform((data) => ({ ...data, company_id: selectedCompany.value.id })).post('/lupa-password-perusahaan', {
            onSuccess: () => {
        ...
        <div class="flex-1 grid grid-cols-1 lg:grid-cols-2 min-w-0">
            <div class="relative overflow-hidden bg-gradient-to-br from-sky-600 via-sky-700 to-indigo-700 flex items-center justify-center px-4 sm:px-6 py-16 lg:py-0 min-w-0">
                ...
                <div class="relative text-center text-white max-w-sm">
                    ...
                    <p class="text-sky-100 leading-relaxed text-sm md:text-base">
                        Pilih perusahaan Anda dan masukkan email untuk menerima link reset password.
                    </p>
                </div>
            </div>
                    ...
                    <div class="rounded-2xl bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-8 shadow-lg">
                        <div class="text-center mb-6">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                {{ isResetMode ? 'Masukkan password baru untuk akun perusahaan Anda.' : 'Masukkan email dan perusahaan Anda untuk menerima link reset.' }}
                            </p>
                        </div>
                            ...
                            <div v-if="!isResetMode">
                                ...
                                <div class="relative">
                                    ...
                                    <input
                                        v-model="form.email"
                                        type="email"
                                        placeholder="perusahaan@contoh.com"
                                        required
                                        :class="['w-full pl-10 pr-4 py-2.5 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm outline-none transition-colors', form.errors.email ? 'border-red-400 focus:ring-red-500/30 focus:border-red-500' : 'border-gray-300 dark:border-gray-700 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']"
                                    />
                                </div>
...
<p v-if="form.errors.email" class="text-red-500 text-xs mt-1">{{ form.errors.email }}</p>
                            ...
                            <template v-else>
                                <div class="rounded-lg bg-sky-50 dark:bg-sky-900/20 border border-sky-200 dark:border-sky-800 p-3 text-sm">
                                    <p class="text-sky-700 dark:text-sky-300"><i class="fas fa-info-circle mr-1.5"></i>Email: <strong>{{ props.email }}</strong></p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input
                                            ...
                                            required
                                            :class="['w-full pl-10 pr-10 py-2.5 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm outline-none transition-colors', form.errors.password ? 'border-red-400 focus:ring-red-500/30 focus:border-red-500' : 'border-gray-300 dark:border-gray-700 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']"
                                        />
                                    <button type="button" @click="showPassword = !showPassword" class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-sky-500 dark:hover:text-sky-400 transition-colors" :title="showPassword ? 'Sembunyikan' : 'Tampilkan'"><i :class="['fas text-sm', showPassword ? 'fa-eye-slash' : 'fa-eye']"></i></button>
                                    </div>
...
<p v-if="form.errors.password" class="text-red-500 text-xs mt-1">{{ form.errors.password }}</p>
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input
                                            ...
                                            required
                                            :class="['w-full pl-10 pr-10 py-2.5 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm outline-none transition-colors', form.errors.password_confirmation ? 'border-red-400 focus:ring-red-500/30 focus:border-red-500' : 'border-gray-300 dark:border-gray-700 focus:ring-2 focus:ring-sky-500 focus:border-sky-500']"
                                        />
                                    <button type="button" @click="showPassword = !showPassword" class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-sky-500 dark:hover:text-sky-400 transition-colors" :title="showPassword ? 'Sembunyikan' : 'Tampilkan'"><i :class="['fas text-sm', showPassword ? 'fa-eye-slash' : 'fa-eye']"></i></button>
                                    </div>
                                </div>
                            ...
                            <button
                                ...
                                class="w-full py-2.5 bg-gradient-to-r from-sky-500 to-indigo-600 text-white font-semibold rounded-lg shadow-md hover:shadow-lg hover:scale-[1.01] active:scale-[0.99] transition-all flex items-center justify-center disabled:opacity-50"
                            >
                        ...
                        <div class="mt-6 text-center text-sm">
                            <Link href="/login-perusahaan" class="text-sky-600 dark:text-sky-400 hover:underline">
