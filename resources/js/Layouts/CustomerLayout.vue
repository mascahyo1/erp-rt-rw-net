  ...
  <div class="min-h-screen flex bg-gray-100 dark:bg-gray-950 transition-colors">
    ...
    <div class="flex-1 flex flex-col min-w-0 min-h-screen lg:ml-64">
          ...
          <div class="flex items-center gap-2">
            ...
            <div class="relative">
              <button @click="profileDropdownOpen = !profileDropdownOpen" class="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                <i class="fas fa-user-circle text-lg"></i><span class="hidden sm:inline font-medium">Pelanggan</span><i :class="['fas text-xs transition-transform', profileDropdownOpen ? 'fa-chevron-up' : 'fa-chevron-down']"></i>
              </button>
              <Transition name="dropdown"><div v-show="profileDropdownOpen" @click="profileDropdownOpen = false" class="absolute right-0 mt-2 w-48 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-lg py-1 z-50">
                <Link href="/customer/profil-saya" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-user w-4 text-center"></i> Profil Saya</Link>
      ...
      <main class="flex-1 py-6 px-4 sm:px-6 lg:px-8 w-full max-w-full overflow-x-hidden">
        <Transition name="page" mode="out-in"><slot /></Transition>
      </main>
    </div>
  </div>
</template>
