...
<template>
  <div class="min-h-screen flex bg-gray-100 dark:bg-gray-950 transition-colors">
    ...
    <aside :class="['fixed inset-y-0 left-0 z-40 flex flex-col bg-gradient-to-b from-amber-700 via-orange-800 to-amber-900 text-white transition-all duration-300', sidebarOpen ? 'w-64' : '-translate-x-full lg:translate-x-0 lg:w-64']">
      <div class="flex items-center justify-between h-16 px-4 border-b border-white/10 shrink-0">
        <Link href="/karyawan/dashboard" class="flex items-center gap-2"><div class="w-8 h-8 bg-white/20 rounded-lg flex items-center justify-center shrink-0"><i class="fas fa-user-tie text-white text-sm"></i></div><span class="font-bold text-sm whitespace-nowrap">Karyawan</span></Link>
      ...
      <nav class="flex-1 overflow-y-auto py-4 px-2 space-y-1">
        <Link v-for="item in menuItems" :key="item.href" :href="item.href" @click="closeMobileMenu" :class="['flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors', isActive(item.href) ? 'bg-white/20 text-white' : 'text-amber-100 hover:bg-white/10 hover:text-white']">
          <i :class="['fas', item.icon, 'w-5 text-center shrink-0']"></i><span>{{ item.label }}</span>
        </Link>
      </nav>
      <div class="p-4 border-t border-white/10">
        <Link href="/" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-amber-100 hover:bg-white/10 hover:text-white transition-colors"><i class="fas fa-arrow-left w-5 text-center shrink-0"></i><span>Kembali ke Landing</span></Link>
      </div>
    </aside>
          ...
          <div class="flex items-center gap-2">
            ...
            <div class="relative">
              <button @click="profileDropdownOpen = !profileDropdownOpen" class="flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                <i class="fas fa-user-circle text-lg"></i><span class="hidden sm:inline font-medium">Karyawan</span><i :class="['fas text-xs transition-transform', profileDropdownOpen ? 'fa-chevron-up' : 'fa-chevron-down']"></i>
              </button>
              <Transition name="dropdown"><div v-show="profileDropdownOpen" @click="profileDropdownOpen = false" class="absolute right-0 mt-2 w-48 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-lg py-1 z-50">
                <Link href="/karyawan/profil-saya" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"><i class="fas fa-user w-4 text-center"></i> Profil Saya</Link>
                <div class="border-t border-gray-100 dark:border-gray-700 my-1"></div>
                <Link href="/" class="flex items-center gap-2 px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"><i class="fas fa-sign-out-alt w-4 text-center"></i> Logout</Link>
              </div></Transition>
            </div>
          </div>
        </div>
