 ...
 *
 * Module target: Customer (karyawan-customer.{list,create,edit,detail,delete,restore})
 *
 ...
 */
class CustomerPermissionTest extends DuskTestCase
{
    ...
    public function test_01_list_only_sees_sidebar_and_table_no_action_buttons(): void
    {
        ...
        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'employee')
                ...
                ->pause(800)
                ->screenshot('karyawan/customer-permission/01-list/01-sidebar')
                ->assertSee('Dashboard')
                ->assertSee('Customer')
                ->assertDontSee('Tagihan');

            $browser->visit('/karyawan/customer?per_page=100')
                ->pause(800)
                ->screenshot('karyawan/customer-permission/01-list/02-table');
            ...
            $browser->visit('/karyawan/customer/create')
                ->pause(500)
                ->assertSee('403');
        });
    }
    ...
    {
        $user = $this->createEmployeeWithPerms(['karyawan-customer.list', 'karyawan-customer.create']);
        ...
        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'employee')
                ...
                ->pause(800)
                ->screenshot('karyawan/customer-permission/02-create/01-before');
            ...
            $browser->press('Tambah')
                ->pause(500)
                ->screenshot('karyawan/customer-permission/02-create/02-modal')
    ...
    {
        $user = $this->createEmployeeWithPerms(['karyawan-customer.list', 'karyawan-customer.edit']);
        ...
        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'employee')
                ->visit('/karyawan/customer?per_page=100')
                ->pause(800)
                ->screenshot('karyawan/customer-permission/03-edit/01-table');
            ...
            $browser->pause(500)
                ->screenshot('karyawan/customer-permission/03-edit/02-bulk-active');

            $browser->assertDontSee('Hapus');

            $browser->script("document.querySelector('a[title=\"Edit\"], button[title=\"Edit\"]').click()");
            $browser->pause(500)
                ->screenshot('karyawan/customer-permission/03-edit/03-modal')
    ...
    {
        $user = $this->createEmployeeWithPerms(['karyawan-customer.list', 'karyawan-customer.detail']);
        ...
        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'employee')
                ->visit('/karyawan/customer?per_page=100')
                ->pause(800)
                ->screenshot('karyawan/customer-permission/04-detail/01-table');
            ...
            $browser->pause(500)
                ->screenshot('karyawan/customer-permission/04-detail/02-modal')
    ...
    {
        $user = $this->createEmployeeWithPerms(['karyawan-customer.list', 'karyawan-customer.delete']);
        ...
        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'employee')
                ->visit('/karyawan/customer?per_page=100')
                ->pause(800)
                ->screenshot('karyawan/customer-permission/05-delete/01-table');

            $browser->assertPresent('button[title="Hapus"], a[title="Hapus"]');

            $browser->script("document.querySelectorAll('tbody input[type=\"checkbox\"]')[0].click()");
            $browser->pause(500)
                ->screenshot('karyawan/customer-permission/05-delete/02-bulk-delete');

            $browser->assertSee('data dipilih');

            $browser->script("document.querySelector('button[title=\"Hapus\"], a[title=\"Hapus\"]').click()");
            $browser->pause(500)
                ->screenshot('karyawan/customer-permission/05-delete/03-delete-modal')
    ...
    {
        $user = $this->createEmployeeWithPerms(['karyawan-customer.list', 'karyawan-customer.restore']);

        $trashCustomer = Customer::factory()->create([
            'company_id' => $user->company_id,
        ]);
        $trashCustomer->delete();
        self::$cleanupUserIds[] = $trashCustomer->id;

        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'employee')
                ->visit('/karyawan/customer?terhapus=ya&per_page=100')
                ->pause(800)
                ->screenshot('karyawan/customer-permission/06-restore/01-trash-tab');

            $browser->assertPresent('button[title="Pulihkan"], a[title="Pulihkan"]');

            $browser->script("document.querySelectorAll('tbody input[type=\"checkbox\"]')[0].click()");
            $browser->pause(500)
                ->screenshot('karyawan/customer-permission/06-restore/02-bulk-restore');
    ...
    public function test_07_no_perms_all_routes_forbidden(): void
    {
        ...
        $this->browse(function (Browser $browser) use ($user) {
            ...
            $browser->visit('/karyawan/dashboard')
                ->pause(500)
                ->screenshot('karyawan/customer-permission/07-forbidden/01-sidebar-only-dashboard')
                ->assertSee('Dashboard')
                ->assertDontSee('Customer');

            $browser->visit('/karyawan/customer')
                ->pause(500)
                ->screenshot('karyawan/customer-permission/07-forbidden/02-get-forbidden')
                ->assertSee('403');
        });
    }
