 ...
 *
 * Module target: Role Perusahaan (role-perusahaan.{list,create,edit,delete,restore})
 *
 ...
 */
class RolePerusahaanPermissionTest extends DuskTestCase
{
    ...
    {
        $user = $this->createUserWithPerms(['role-perusahaan.list']);

        Role::create(['scope' => 'operator_saas', 'name' => 'Role Perusahaan A', 'is_active' => true, 'display_order' => 1]);

        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'web')
                ->visit('/operator-saas/dashboard')
                ->pause(800)
                ->screenshot('operator-saas/role-perusahaan-permission/01-list/01-sidebar')
                ->assertSee('Dashboard')
                ->assertSee('Role Perusahaan')
                ->assertDontSee('Admin SaaS');

            $browser->visit('/operator-saas/role-perusahaan?per_page=100')
                ->pause(800)
                ->screenshot('operator-saas/role-perusahaan-permission/01-list/02-table');
            ...
            // No edit/delete/restore button per row
            $browser->assertDontSee('Edit');
            $browser->assertDontSee('Hapus');

            // Direct POST store → 403
            $browser->visit('/operator-saas/role-perusahaan/create')
                ->pause(500)
                ->assertSee('403');
        });
    }
    ...
    {
        $user = $this->createUserWithPerms(['role-perusahaan.list', 'role-perusahaan.create']);

        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'web')
                ->visit('/operator-saas/role-perusahaan?per_page=100')
                ->pause(800)
                ->screenshot('operator-saas/role-perusahaan-permission/02-create/01-before');
            ...
            $browser->press('Tambah')
                ->pause(500)
                ->screenshot('operator-saas/role-perusahaan-permission/02-create/02-modal')
    ...
    {
        $user = $this->createUserWithPerms(['role-perusahaan.list', 'role-perusahaan.edit']);

        Role::create(['scope' => 'operator_saas', 'name' => 'Edit Role A', 'is_active' => true, 'display_order' => 1]);

        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'web')
                ->visit('/operator-saas/role-perusahaan?per_page=100')
                ->pause(800)
                ->screenshot('operator-saas/role-perusahaan-permission/03-edit/01-table');
            ...
            $browser->pause(500)
                ->screenshot('operator-saas/role-perusahaan-permission/03-edit/02-bulk-active');
            ...
            $browser->pause(500)
                ->screenshot('operator-saas/role-perusahaan-permission/03-edit/03-modal')
                ->assertSee('Edit Role');
    ...
    {
        $user = $this->createUserWithPerms(['role-perusahaan.list', 'role-perusahaan.delete']);

        Role::create(['scope' => 'operator_saas', 'name' => 'Delete Role A', 'is_active' => true, 'display_order' => 1]);

        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'web')
                ->visit('/operator-saas/role-perusahaan?per_page=100')
                ->pause(800)
                ->screenshot('operator-saas/role-perusahaan-permission/05-delete/01-table');
            ...
            $browser->script("document.querySelectorAll('tbody input[type=\"checkbox\"]')[0].click()");
            $browser->pause(500)
                ->screenshot('operator-saas/role-perusahaan-permission/05-delete/02-bulk-delete');
            ...
            $browser->pause(500)
                ->screenshot('operator-saas/role-perusahaan-permission/05-delete/03-delete-modal')
    ...
    {
        $user = $this->createUserWithPerms(['role-perusahaan.list', 'role-perusahaan.restore']);

        // Create + soft-delete a Role so trash tab has data
        $trashRole = Role::create([
            'scope' => 'operator_saas',
            'name' => 'To Restore Role',
            'is_active' => true,
            'display_order' => 1,
        ]);
        $trashRole->delete();
        self::$cleanupRoleIds[] = $trashRole->id;

        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'web')
                ->visit('/operator-saas/role-perusahaan?terhapus=ya&per_page=100')
                ->pause(800)
                ->screenshot('operator-saas/role-perusahaan-permission/06-restore/01-trash-tab');
            ...
            $browser->script("document.querySelectorAll('tbody input[type=\"checkbox\"]')[0].click()");
            $browser->pause(500)
                ->screenshot('operator-saas/role-perusahaan-permission/06-restore/02-bulk-restore');
    ...
    public function test_07_no_perms_all_routes_forbidden(): void
    {
        ...
        $this->browse(function (Browser $browser) use ($user) {
            ...
            $browser->visit('/operator-saas/dashboard')
                ->pause(500)
                ->screenshot('operator-saas/role-perusahaan-permission/07-forbidden/01-sidebar-only-dashboard')
                ->assertSee('Dashboard')
                ->assertDontSee('Role Perusahaan');

            // Direct GET → 403
            $browser->visit('/operator-saas/role-perusahaan')
                ->pause(500)
                ->screenshot('operator-saas/role-perusahaan-permission/07-forbidden/02-get-forbidden')
                ->assertSee('403');
        });
    }
