 ...
 *
 * Module target: Langganan Customer (langganan.{list,create,edit,detail,delete,restore})
 *
...
class LanggananPermissionTest extends DuskTestCase
{
    ...
    {
        $user = $this->createUserWithPerms(['langganan.list']);

        // Create table data with matching company_id
        $customer = Customer::factory()->create([
            'company_id' => $user->company_id,
            'name' => 'List Cust',
            'email' => 'langganan.list@rtrwnet.id',
            'is_active' => true,
        ]);
        ...
        CustInternet::create([
            ...
            'account_number' => 'LIST001',
            'internet_status' => 'active',
        ]);
        self::$cleanupCustInternetIds[] = $customer->id;

        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'admin-company')
                ->visit('/operator-perusahaan/dashboard')
                ->pause(800)
                ->screenshot('operator-perusahaan/langganan-permission/01-list/01-sidebar')
                ->assertSee('Dashboard')
                ->assertSee('Langganan Customer')
                ->assertDontSee('Admin Perusahaan')
                ->assertDontSee('Karyawan');

            $browser->visit('/operator-perusahaan/langganan-customer?per_page=100')
                ->pause(800)
                ->screenshot('operator-perusahaan/langganan-permission/01-list/02-table');
            ...
            $browser->visit('/operator-perusahaan/langganan-customer/create')
                ->pause(500)
                ->assertSee('403');
        });
    }
    ...
    {
        $user = $this->createUserWithPerms(['langganan.list', 'langganan.create']);

        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'admin-company')
                ->visit('/operator-perusahaan/langganan-customer?per_page=100')
                ->pause(800)
                ->screenshot('operator-perusahaan/langganan-permission/02-create/01-before');
            ...
            $browser->press('Tambah')
                ->pause(500)
                ->screenshot('operator-perusahaan/langganan-permission/02-create/02-modal')
    ...
    {
        $user = $this->createUserWithPerms(['langganan.list', 'langganan.edit']);

        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'admin-company')
                ->visit('/operator-perusahaan/langganan-customer?per_page=100')
                ->pause(800)
                ->screenshot('operator-perusahaan/langganan-permission/03-edit/01-table');
            ...
            $browser->pause(500)
                ->screenshot('operator-perusahaan/langganan-permission/03-edit/02-bulk-active');
            ...
            $browser->pause(500)
                ->screenshot('operator-perusahaan/langganan-permission/03-edit/03-modal')
    ...
    {
        $user = $this->createUserWithPerms(['langganan.list', 'langganan.detail']);

        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'admin-company')
                ->visit('/operator-perusahaan/langganan-customer?per_page=100')
                ->pause(800)
                ->screenshot('operator-perusahaan/langganan-permission/04-detail/01-table');
            ...
            $browser->pause(500)
                ->screenshot('operator-perusahaan/langganan-permission/04-detail/02-modal')
    ...
    {
        $user = $this->createUserWithPerms(['langganan.list', 'langganan.delete']);

        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'admin-company')
                ->visit('/operator-perusahaan/langganan-customer?per_page=100')
                ->pause(800)
                ->screenshot('operator-perusahaan/langganan-permission/05-delete/01-table');
            ...
            $browser->script("document.querySelectorAll('tbody input[type=\"checkbox\"]')[0].click()");
            $browser->pause(500)
                ->screenshot('operator-perusahaan/langganan-permission/05-delete/02-bulk-delete');
            ...
            $browser->pause(500)
                ->screenshot('operator-perusahaan/langganan-permission/05-delete/03-delete-modal')
    ...
    {
        $user = $this->createUserWithPerms(['langganan.list', 'langganan.restore']);

        // Create + soft-delete a CustInternet so trash tab has data
        $customer = Customer::factory()->create([
            'company_id' => $user->company_id,
            'name' => 'To Restore Langganan',
            'email' => 'torestore.lg@rtrwnet.id',
            'is_active' => true,
        ]);

        $trashCustInternet = CustInternet::create([
            'customer_id' => $customer->id,
            'internet_package_id' => null,
            'account_number' => 'RESTORE001',
            'internet_status' => 'active',
        ]);
        $trashCustInternet->delete();
        self::$cleanupCustInternetIds[] = $customer->id;

        $this->browse(function (Browser $browser) use ($user) {
            $browser->loginAs($user, 'admin-company')
                ->visit('/operator-perusahaan/langganan-customer?terhapus=ya&per_page=100')
                ->pause(800)
                ->screenshot('operator-perusahaan/langganan-permission/06-restore/01-trash-tab');
            ...
            $browser->script("document.querySelectorAll('tbody input[type=\"checkbox\"]')[0].click()");
            $browser->pause(500)
                ->screenshot('operator-perusahaan/langganan-permission/06-restore/02-bulk-restore');
    ...
    public function test_07_no_perms_all_routes_forbidden(): void
    {
        ...
        $this->browse(function (Browser $browser) use ($user) {
            ...
            $browser->visit('/operator-perusahaan/dashboard')
                ->pause(500)
                ->screenshot('operator-perusahaan/langganan-permission/07-forbidden/01-sidebar-only-dashboard')
                ->assertSee('Dashboard')
            ...
            // Direct GET → 403
            $browser->visit('/operator-perusahaan/langganan-customer')
                ->pause(500)
                ->screenshot('operator-perusahaan/langganan-permission/07-forbidden/02-get-forbidden')
                ->assertSee('403');
        });
    }
    ...
    {
        CustInternet::whereIn('customer_id', self::$cleanupCustInternetIds)->forceDelete();
        Customer::whereIn('id', self::$cleanupCustInternetIds)->forceDelete();
