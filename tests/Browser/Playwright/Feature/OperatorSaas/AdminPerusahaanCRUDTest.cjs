...
class AdminPerusahaanCRUDTest {
    ...
    async runAllTests() {
        console.log('========================================');
        console.log('Admin Perusahaan CRUD Tests - Playwright');
        console.log('========================================\n');

        try {
            await this.helper.launch();
            await this.helper.loginAsAdminSaaS('admin-saas@rtrwnet.id', 'password123');
            await this.helper.screenshot('OperatorSaas/AdminPerusahaan/TestCRUD/00-login');
            ...
            await this.helper.screenshot('OperatorSaas/AdminPerusahaan/TestCRUD/XX-fatal');
        } finally {
    ...
    async safeTest(name, fn) {
        try {
            ...
            await this.helper.screenshot(`OperatorSaas/AdminPerusahaan/TestCRUD/XX-${name.replace(/\s/g, '-')}`);
        }
    }
    ...
    async test_01_page_renders() {
        await this.safeTest('test_01_page_renders', async () => {
            ...
            await this.helper.waitForText('Admin', 10000);
    ...
    async test_02_search() {
        await this.safeTest('test_02_search', async () => {
            ...
            await this.helper.screenshot('OperatorSaas/AdminPerusahaan/TestCRUD/02-search');
        });
    }
    ...
    async test_03_filter_status() {
        await this.safeTest('test_03_filter_status', async () => {
            ...
            await this.helper.screenshot('OperatorSaas/AdminPerusahaan/TestCRUD/03-filter');
        });
    }
}

const test = new AdminPerusahaanCRUDTest();
