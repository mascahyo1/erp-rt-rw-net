 ...
 * Ganti Pemetaan Admin Perusahaan, ikuti 15 langkah checklist STANDARDS Â§7.3
 *
 * Cara pakai:
...
const RESULT_DIR = path.join(__dirname, '..', 'Feature', 'result', 'Feature', 'DeepVerifyPemetaanAdminPerusahaan');
...
async function main() {
  ...
  // Pemetaan Admin Perusahaan: pilih perusahaan via .fa-building jika diperlukan
  ...
  console.log('\n[2] Navigasi');
  await page.goto(`${BASE}/operator-saas/pemetaan-admin-perusahaan`, { waitUntil: 'domcontentloaded' });
  ...
  // Pemetaan Admin Perusahaan: picks type, fill, waitForResponse, assert 200, screenshot
