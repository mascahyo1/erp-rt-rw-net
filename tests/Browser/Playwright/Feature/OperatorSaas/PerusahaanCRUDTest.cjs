...
class PerusahaanCRUDTest {
    ...
    async runAllTests() {
        console.log('========================================');
        console.log('Perusahaan CRUD Tests - Playwright');
        console.log('========================================\n');
        ...
        try {
            ...
            await this.helper.screenshot('OperatorSaas/Perusahaan/TestCRUD/XX-fatal');
        } finally {
    ...
    async safeTest(name, fn) {
        try {
            ...
            await this.helper.screenshot(`OperatorSaas/Perusahaan/TestCRUD/XX-${name.replace(/\s/g, '-')}`);
        }
    }
    ...
    async test_01_page_renders() {
        await this.safeTest('test_01_page_renders', async () => {
            ...
            await this.helper.page.goto(`${BASE}/operator-saas/perusahaan`);
    ...
    async test_02_search() {
        await this.safeTest('test_02_search', async () => {
            await this.helper.page.goto(`${BASE}/operator-saas/perusahaan?per_page=100`);
            await this.helper.page.waitForTimeout(3000);
...
const test = new PerusahaanCRUDTest();
