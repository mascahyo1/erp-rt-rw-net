 ...
 * Ganti Admin Role SaaS, ikuti 15 langkah checklist STANDARDS Â§7.3
 *
 * Cara pakai:
...
const RESULT_DIR = path.join(__dirname, '..', 'Feature', 'result', 'Feature', 'DeepVerifyAdminRoleSaaS');
...
async function main() {
  ...
  // Admin Role SaaS: pilih perusahaan via .fa-building jika diperlukan
  ...
  console.log('\n[2] Navigasi');
  await page.goto(`${BASE}/operator-saas/admin-role-saas`, { waitUntil: 'domcontentloaded' });
  ...
  // Admin Role SaaS: picks type, fill, waitForResponse, assert 200, screenshot
