 ...
 * Ganti Verifikasi Email, ikuti 15 langkah checklist STANDARDS Â§7.3
 *
 * Cara pakai:
...
const RESULT_DIR = path.join(__dirname, '..', 'Feature', 'result', 'Feature', 'DeepVerifyVerifikasiEmail');
...
async function main() {
  ...
  // Verifikasi Email: pilih perusahaan via .fa-building jika diperlukan
  ...
  console.log('\n[2] Navigasi');
  await page.goto(`${BASE}/customer/verifikasi-email`, { waitUntil: 'domcontentloaded' });
  ...
  // Verifikasi Email: picks type, fill, waitForResponse, assert 200, screenshot
