...
class LoginPelangganTest {
    constructor() {
        ...
        this.email = 'pelanggan@rtrwnet.id';
    ...
    async runAllTests() {
        ...
        try {
            ...
            await this.helper.screenshot('Pelanggan/Login/XX-fatal');
        } finally {
    ...
    async safeTest(name, fn) {
        try {
            ...
            await this.helper.screenshot(`Pelanggan/Login/XX-error-${name.replace(/\s/g, '-')}`);
        }
    }
    ...
    async test_page_renders() {
        await this.safeTest('test_page_renders', async () => {
            ...
            if (!pageText.includes('Masuk')) {
                throw new Error('Page should show "Masuk" text');
            }
        });
    }
