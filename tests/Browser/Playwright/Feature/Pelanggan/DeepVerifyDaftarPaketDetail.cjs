 ...
 *
 * Copy file ini untuk fitur baru: cp DeepVerifyTemplate.cjs ../Feature/{Pelanggan}/DeepVerify{NamaFitur}.cjs
 * Ganti Daftar Paket Detail, ikuti 15 langkah checklist STANDARDS Â§7.3
 *
 * Cara pakai:
...
const RESULT_DIR = path.join(__dirname, '..', 'Feature', 'result', 'Pelanggan', 'DeepVerifyDaftarPaketDetail');
...
async function main() {
  ...
  // Daftar Paket Detail: pilih perusahaan via .fa-building jika diperlukan
  ...
  console.log('\n[2] Navigasi');
  await page.goto(`${BASE}/customer/daftar-paket-detail`, { waitUntil: 'domcontentloaded' });
  ...
  // Daftar Paket Detail: picks type, fill, waitForResponse, assert 200, screenshot
