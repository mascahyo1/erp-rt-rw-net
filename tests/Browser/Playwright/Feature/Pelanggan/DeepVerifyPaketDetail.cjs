 ...
 * Ganti Paket Detail, ikuti 15 langkah checklist STANDARDS Â§7.3
 *
 * Cara pakai:
...
const RESULT_DIR = path.join(__dirname, '..', 'Feature', 'result', 'Pelanggan', 'DeepVerifyPaketDetail');
...
async function main() {
  ...
  // Paket Detail: pilih perusahaan via .fa-building jika diperlukan
  ...
  console.log('\n[2] Navigasi');
  await page.goto(`${BASE}/customer/paket-detail`, { waitUntil: 'domcontentloaded' });
  ...
  // Paket Detail: picks type, fill, waitForResponse, assert 200, screenshot
