...
class LanggananCustomerViewTest {
    constructor() {
    ...
    async runAllTests() {
        console.log('========================================');
        console.log('Karyawan Langganan Customer View Tests - Playwright');
        console.log('========================================\n');
        ...
        try {
            ...
            await this.helper.screenshot('Karyawan/LanggananCustomer/TestView/00-after-login');
            ...
            await this.helper.screenshot('Karyawan/LanggananCustomer/TestView/XX-fatal');
        } finally {
    ...
    async safeTest(name, fn) {
        try {
            ...
            await this.helper.screenshot(`Karyawan/LanggananCustomer/TestView/XX-${name.replace(/\s/g, '-')}`);
        }
    }

    async test_01_page_renders() {
        await this.safeTest('test_01_page_renders', async () => {
            await this.helper.page.goto(`${this.baseUrl}/karyawan/langganan-customer`);
            await this.helper.page.waitForTimeout(3000);
            await this.helper.screenshot('Karyawan/LanggananCustomer/TestView/01-page');
...
const test = new LanggananCustomerViewTest();
