...
class LoginKaryawan {
    ...
    async runAllTests() {
        console.log('========================================');
        console.log('Karyawan Login Tests - Playwright');
        console.log('========================================\n');
        ...
        try {
            ...
            await this.helper.screenshot('Karyawan/Login/XX-fatal-error');
        } finally {
    ...
    async test_page_renders() {
        ...
        try {
            ...
            await this.helper.waitForText('Login Karyawan', 10000);
            ...
            await this.helper.screenshot('Karyawan/Login/XX-error');
        }
    }
    ...
    async test_wrong_password_shows_error() {
        ...
        try {
            ...
            await this.helper.screenshot('Karyawan/Login/02-before-submit');

            await this.helper.fill('input[type="email"]', 'test@ karyawan.com');
            ...
            await this.helper.screenshot('Karyawan/Login/03-error-shown');

            const currentUrl = this.helper.getCurrentUrl();
            if (!currentUrl.includes('login-karyawan')) {
    ...
    async test_inactive_user_rejected() {
        ...
        try {
            await this.helper.page.goto(`${this.baseUrl}/login-karyawan`);
            await this.helper.waitForSelector('button[type="submit"]', 10000);
            await this.helper.screenshot('Karyawan/Login/04-inactive-before');

            await this.helper.fill('input[type="email"]', 'inactive@ karyawan.com');
            await this.helper.fill('input[type="password"]', 'password');
            await this.helper.click('button[type="submit"]');
            await this.helper.pause(2000);
            await this.helper.screenshot('Karyawan/Login/05-inactive-after');
