...
`;
const tmpPhp = path.join(PROJECT_ROOT, '.claude', 'tmp_karyawan_login_test.php');
...
class LoginKaryawanTest {
    ...
    async runAllTests() {
        console.log('========================================');
        console.log('Karyawan Login Tests - DEEP');
        console.log('========================================\n');
        ...
        try {
            ...
            const inactive = createInactiveUser('karyawan', this.companyId);
            this.testUsers.push({ type: 'karyawan', id: inactive.id });
            ...
            try { await this.helper.screenshot('Karyawan/Login/XX-fatal'); } catch (e) {}
        } finally {
    ...
    async safeTest(name, fn) {
        try {
            ...
            try { await this.helper.screenshot(`Karyawan/Login/XX-error-${name.replace(/\s/g, '-')}`); } catch (s) {}
        }
    }
    ...
    async test_page_renders() {
        await this.safeTest('test_page_renders', async () => {
            ...
            await this.helper.screenshot('Karyawan/Login/01-page');
            ...
            if (!bodyText.includes('Login Karyawan')) {
                throw new Error('Heading "Login Karyawan" tidak ada');
            }
        });
    }
    ...
    async test_wrong_password_shows_error() {
        await this.safeTest('test_wrong_password_shows_error', async () => {
            ...
            await this.helper.fill('input[type="email"]', 'siti@netsejahtera.com');
            ...
            await this.helper.screenshot('Karyawan/Login/02-wrong-password');

            const url = this.helper.getCurrentUrl();
            if (!url.includes('login-karyawan')) {
    ...
    async test_company_required() {
        await this.safeTest('test_company_required', async () => {
            ...
            await this.helper.screenshot('Karyawan/Login/03-company-required');
    ...
    async test_company_mismatch() {
        await this.safeTest('test_company_mismatch', async () => {
            ...
            await this.helper.click('button[type="submit"]');
            await this.helper.page.waitForTimeout(2500);
            await this.helper.screenshot('Karyawan/Login/04-company-mismatch');
    ...
    async test_inactive_user_rejected() {
        await this.safeTest('test_inactive_user_rejected', async () => {
            const isActive = phpExec(`
                $u = \\App\\Models\\Employee::find('${this.testUsers[0].id}');
            ...
            await this.helper.click('button[type="submit"]');
            await this.helper.page.waitForTimeout(2500);
            await this.helper.screenshot('Karyawan/Login/05-inactive-rejected');
    ...
    async test_guest_redirect_to_login() {
        await this.safeTest('test_guest_redirect_to_login', async () => {
            await this.helper.page.goto(BASE + '/karyawan/dashboard');
            await this.helper.page.waitForTimeout(1500);
            await this.helper.screenshot('Karyawan/Login/06-guest-redirect');
            const url = this.helper.getCurrentUrl();
            if (!url.includes('login-karyawan')) {
...
const test = new LoginKaryawanTest();
