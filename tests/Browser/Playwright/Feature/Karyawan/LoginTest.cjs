...
class LoginKaryawanTest {
    constructor() {
        ...
        this.email = 'karyawan@rtrwnet.id';
    ...
    async runAllTests() {
        ...
        try {
            ...
            await this.helper.screenshot('Karyawan/Login/XX-fatal');
        } finally {
    ...
    async safeTest(name, fn) {
        try {
            ...
            await this.helper.screenshot(`Karyawan/Login/XX-error-${name.replace(/\s/g, '-')}`);
        }
    }
    ...
    async test_page_renders() {
        await this.safeTest('test_page_renders', async () => {
            ...
            if (!pageText.includes('Login Karyawan')) {
                throw new Error('Page should show "Login Karyawan" text');
            }
        });
    }
