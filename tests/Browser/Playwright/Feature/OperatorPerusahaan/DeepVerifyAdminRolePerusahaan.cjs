 ...
 * Ganti Admin Role Perusahaan, ikuti 15 langkah checklist STANDARDS Â§7.3
 *
 * Cara pakai:
...
const RESULT_DIR = path.join(__dirname, '..', 'Feature', 'result', 'Feature', 'DeepVerifyAdminRolePerusahaan');
...
async function main() {
  ...
  // Admin Role Perusahaan: pilih perusahaan via .fa-building jika diperlukan
  ...
  console.log('\n[2] Navigasi');
  await page.goto(`${BASE}/operator-perusahaan/admin-role-perusahaan`, { waitUntil: 'domcontentloaded' });
  ...
  // Admin Role Perusahaan: picks type, fill, waitForResponse, assert 200, screenshot
