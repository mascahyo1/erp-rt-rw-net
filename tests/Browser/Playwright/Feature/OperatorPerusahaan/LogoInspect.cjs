...
(async () => {
    ...
    // Go to Perusahaan
    await page.goto(BASE + '/operator-saas/perusahaan');
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(2000);
    await page.screenshot({ path: path.join(__dirname, '..', 'result', '_view-saas-list.png') });

    // Click Tambah
    const tambahBtn = page.locator('button:has-text("Tambah")').first();
    await tambahBtn.click();
    await page.waitForTimeout(1500);
    await page.screenshot({ path: path.join(__dirname, '..', 'result', '_view-saas-create.png') });
    ...
    if (await editBtn.count() > 0) {
        await editBtn.click();
        await page.waitForTimeout(1500);
        await page.screenshot({ path: path.join(__dirname, '..', 'result', '_view-saas-edit.png') });
    ...
    if (await detailBtn.count() > 0) {
        ...
        await page.waitForTimeout(1500);
        await page.screenshot({ path: path.join(__dirname, '..', 'result', '_view-saas-detail.png') });
    }
    ...
    await page.goto(BASE + '/operator-perusahaan/perusahaan-saya');
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(2000);
    await page.screenshot({ path: path.join(__dirname, '..', 'result', '_view-saya-list.png') });
    ...
    if (await editSaya.count() > 0) {
        ...
        await page.waitForTimeout(1500);
        await page.screenshot({ path: path.join(__dirname, '..', 'result', '_view-saya-edit.png') });
    }
