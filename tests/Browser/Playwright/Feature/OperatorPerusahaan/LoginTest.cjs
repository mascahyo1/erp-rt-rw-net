...
class LoginOperatorPerusahaanTest {
    constructor() {
        ...
        this.email = 'admin-perusahaan@rtrwnet.id';
        this.password = 'password123';
    }

    async runAllTests() {
        console.log('========================================');
        console.log('Operator Perusahaan Login Tests - Playwright');
        console.log('========================================\n');
        ...
        try {
            ...
            await this.helper.screenshot('OperatorPerusahaan/Login/XX-fatal');
        } finally {
    ...
    async safeTest(name, fn) {
        try {
            ...
            await this.helper.screenshot(`OperatorPerusahaan/Login/XX-error-${name.replace(/\s/g, '-')}`);
        }
    }
    ...
    async test_page_renders() {
        await this.safeTest('test_page_renders', async () => {
            ...
            if (!pageText.includes('Masuk Perusahaan')) {
                throw new Error('Page should show "Masuk Perusahaan" text');
            }
        });
    }
    ...
    async test_inactive_user_rejected() {
        await this.safeTest('test_inactive_user_rejected', async () => {
            await this.helper.page.goto(`${this.baseUrl}/login-perusahaan`);
            await this.helper.waitForSelector('button[type="submit"]', 10000);
            await this.helper.screenshot('OperatorPerusahaan/Login/04-inactive-before');

            await this.helper.fill('input[type="email"]', 'inactive@example.com');
            await this.helper.fill('input[type="password"]', 'password');
            await this.helper.click('button[type="submit"]');
            await this.helper.pause(2000);
            await this.helper.screenshot('OperatorPerusahaan/Login/05-inactive-after');
